...
IMG_LIBERO_STILL = ROOT / "runs/libero_spatial_task0_bringup_20260908/rollout/videos_e8_180/agent_key_01.png"
...
FORMAL_METRICS_JSONL = ROOT / "runs/libero_spatial_task0_bringup_20260908/formal/metrics.jsonl"
...
def load_formal_epoch_rows(path: Path) -> list[dict]:
    """Read the compact validation rows from the formal run.

    The JSONL also contains very large per-window training records.  The
    presentation only needs the epoch summaries, so keep those rows separate
    and make the source of the appendix values explicit.
    """
    rows: list[dict] = []
    if not path.exists():
        return rows
    ...
    for raw in raw_lines:
        try:
            ...
            continue
        if obj.get("kind") != "epoch" or not isinstance(obj.get("validation"), dict):
            continue
        rows.append(obj)
    rows.sort(key=lambda item: num(item.get("epoch"), 0.0))
    return rows
...
def metric_card(slide, label, value, x, y, w, accent=CYAN, sub="", h=0.92):
    ...
    compact = h < 0.82
    add_text(slide, value, x + 0.15, y + 0.31, w - 0.3, 0.24 if compact else 0.31,
             18 if compact else 22, accent, True, FONT_LATIN)
    if sub:
        add_text(slide, sub, x + 0.15, y + h - (0.15 if compact else 0.20), w - 0.3, 0.14, 8.2, MUTED, False, FONT_CN)
...
# --- Coherent mainline draft -----------------------------------------------
#
# The original 22-page review remains below as a reference implementation.  A
# separate, smaller pass is selected with CLEARVLA_COHERENT=1 so the narrative
# can be reviewed without overwriting the earlier artifact.
if os.environ.get("CLEARVLA_COHERENT") == "1":
    prs = Presentation()
    prs.slide_width = Inches(W)
    prs.slide_height = Inches(H)
    BLANK = prs.slide_layouts[6]
    prs.core_properties.title = "ClearVLA · coherent mainline review"
    prs.core_properties.subject = "Environment replay, residual-SAC interface and local structure plan"
    prs.core_properties.author = "ClearVLA"
    _EMBEDDED_VIDEOS.clear()
    _POSTER_CACHE.clear()

    # Page 1 — three work items, presented as one connected chain rather than
    # three parallel marketing blocks.
    ...
    add_text(s, "这段时间，实际改了什么", 0.82, 1.30, 7.35, 0.36, 21, WHITE, True)
    add_text(s, "三件事沿同一条闭环推进：环境、强化学习、局部结构。", 0.84, 1.72, 7.20, 0.25, 11.5, MUTED)
    add_line(s, 1.13, 2.30, 1.13, 6.24, GRID, 2.0)
    chain_rows = [
        ("01", "环境与回放", "三套环境接通观测—动作—回读；用成功/未完成回放定位坐标、时序和起点", CYAN),
        ("02", "强化学习接入", "冻结现有 ClearVLA，在原生动作后接残差 SAC（Soft Actor-Critic）；runner、replay、双 Q 和 checkpoint 已有入口", GREEN),
        ("03", "局部结构修正", "不重画已有主干，只查语言/S、W/P 和动作解码的边界；先 trace，再改归属", WHITE),
    ]
    for i, (num_txt, title, body, col) in enumerate(chain_rows):
        yy = 2.20 + i * 1.28
        shape_oval(s, 0.90, yy, 0.46, 0.46, col)
        add_text(s, num_txt, 0.90, yy + 0.13, 0.46, 0.16, 9.0, NAVY, True, FONT_LATIN, PP_ALIGN.CENTER)
        add_text(s, title, 1.62, yy - 0.01, 2.16, 0.24, 14.2, col, True)
        add_text(s, body, 3.72, yy + 0.01, 4.32, 0.52, 10.3, WHITE, False, FONT_CN)
        if i < len(chain_rows) - 1:
            draw_arrow(s, 1.13, yy + 0.52, 1.13, yy + 1.15, GRID, 0.9)
    shape_rect(s, 8.42, 1.34, 4.12, 5.55, PANEL, 0.10, GRID, 0.7)
    add_text(s, "证据先行", 8.76, 1.64, 3.42, 0.24, 13.5, CYAN, True)
    add_image_cover(s, IMG_SERVER_DRAWER_E4_245, 8.78, 2.08, 3.40, 1.34, frame=True)
    pill(s, "已完成 · CALVIN 单任务", 8.96, 2.25, 2.02, GREEN, NAVY, 7.7)
    add_text(s, "245 步 · 开抽屉", 8.86, 3.56, 3.20, 0.20, 11.5, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_line(s, 8.78, 3.94, 12.18, 3.94, GRID, 0.7)
    add_image_cover(s, IMG_STACK_200, 8.78, 4.20, 1.60, 1.10, frame=True)
    add_image_cover(s, IMG_LIBERO_STILL, 10.58, 4.20, 1.60, 1.10, frame=True)
    add_text(s, "未完成 · StackCube", 8.78, 5.44, 1.60, 0.18, 8.8, RED, True, FONT_CN, PP_ALIGN.CENTER)
    add_text(s, "未成功 · LIBERO", 10.58, 5.44, 1.60, 0.18, 8.8, RED, True, FONT_CN, PP_ALIGN.CENTER)
    add_text(s, "主干接入是已有底座；本轮补的是闭环证据、强化学习接口和局部结构。", 8.78, 6.17, 3.40, 0.34, 8.9, MUTED, False, FONT_CN, PP_ALIGN.CENTER)

    # Page 2 — results are separated before any diagnosis.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "先看结果", "成功回放和未完成回放分开", 2,
               "先看哪些链路已经完成，再回到对应回放解释原因。")
    shape_rect(s, 0.72, 1.70, 5.72, 4.54, PANEL, 0.10, GRID, 0.7)
    pill(s, "已完成", 0.98, 1.92, 0.92, GREEN, NAVY, 8.4)
    add_text(s, "CALVIN · 单任务", 2.12, 1.96, 2.40, 0.20, 11.5, WHITE, True, FONT_CN)
    video_card(s, IMG_SERVER_DRAWER_E4_245, VID_SERVER_DRAWER_E4_245,
               "开抽屉 · 成功", "server · 245 步", 0.98, 2.30, 5.20, 2.88, GREEN, "13.8 s")
    metric_card(s, "判定", "成功", 1.00, 5.42, 1.55, GREEN, "CALVIN", 0.72)
    metric_card(s, "步数", "245", 2.72, 5.42, 1.55, CYAN, "回放长度", 0.72)
    metric_card(s, "来源", "server", 4.44, 5.42, 1.54, ORANGE, "服务器回放", 0.72)
    shape_rect(s, 6.70, 1.70, 5.92, 4.54, PANEL_ALT, 0.10, GRID, 0.7)
    pill(s, "未完成 / 未成功", 6.96, 1.92, 1.55, RED, NAVY, 8.0)
    add_text(s, "自动闭环", 10.62, 1.96, 1.72, 0.20, 11.5, WHITE, True, FONT_CN, PP_ALIGN.RIGHT)
    video_card(s, IMG_LIBERO_STILL, VID_LIBERO_FULL_E4,
               "LIBERO · 未成功", "180 步 · 仅策略", 6.96, 2.30, 2.66, 2.88, RED, "18.6 s")
    video_card(s, IMG_STACK_200, VID_STACK,
               "StackCube · 未完成", "400 步 · base · Δa=0", 9.84, 2.30, 2.52, 2.88, RED, "20.1 s")
    add_text(s, "后续页面按结果分组说明原因。", 0.98, 6.48, 11.36, 0.18, 10.8, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "artifacts/server_media/ · runs/libero_* · runs/stackcube_v2_eval_20260910/", "内嵌视频 · 直接点击播放")

    # Page 3 — environment bridge and feedback loop.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "环境闭环", "三套环境，接到同一条观测—动作—回读链", 3,
               "三套环境共用同一条链，分别提供验证面。")
    video_card(s, IMG_LIBERO_STILL, VID_LIBERO_FULL,
               "LIBERO · 完整回放（未成功）", "180 步 · 观测—动作—回读", 0.72, 1.78, 6.20, 3.72, CYAN, "18.6 s")
    add_text(s, "每轮只执行动作块的第 1 行，执行后的状态和动作回到下一轮。", 0.96, 5.66, 5.72, 0.22, 11.0, WHITE, True, FONT_CN)
    shape_rect(s, 7.30, 1.78, 5.32, 3.72, PANEL, 0.10, GRID, 0.7)
    nodes = [
        ("观测", "RGB / DINO\n状态 + 历史", 7.58, 2.16, 1.30, 0.86, CYAN),
        ("策略", "ClearVLA\n提案 → 细化", 9.18, 2.16, 1.58, 0.86, ORANGE),
        ("执行", "原生动作\n首行", 11.04, 2.16, 1.26, 0.86, GREEN),
        ("回读", "实际动作\n回写下一轮历史", 9.18, 3.70, 1.58, 0.86, WHITE),
    ]
    for title, body, x, y, w, h, col in nodes:
        node(s, title, body, x, y, w, h, PANEL_ALT, col, 12.2, 9.0)
    draw_arrow(s, 8.90, 2.59, 9.10, 2.59, GRID, 0.9)
    draw_arrow(s, 10.78, 2.59, 11.00, 2.59, GRID, 0.9)
    draw_arrow(s, 11.66, 3.05, 10.05, 3.64, GRID, 0.9)
    draw_arrow(s, 9.18, 4.14, 8.12, 4.14, GRID, 0.9)
    add_text(s, "LIBERO · 180 步", 7.58, 4.92, 1.55, 0.18, 9.4, CYAN, True, FONT_CN)
    add_text(s, "CALVIN · 服务器回放", 9.18, 4.92, 1.90, 0.18, 9.4, ORANGE, True, FONT_CN)
    add_text(s, "StackCube · 400 步", 11.04, 4.92, 1.46, 0.18, 9.4, GREEN, True, FONT_CN)
    shape_rect(s, 0.72, 5.88, 11.90, 0.62, PANEL_ALT, 0.08, GRID, 0.6)
    add_text(s, "已把输入、执行、回读和证据身份接在一起；结论以闭环回放为准。", 0.98, 6.08, 11.34, 0.20, 11.4, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "runs/libero_spatial_task0_bringup_20260908/ · artifacts/server_media/ · runs/stackcube_v2_eval_20260910/", "同一条闭环")

    # Page 4 — combine the diagnostic probes into one temporal story.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "单任务定位", "问题按出现顺序排查：坐标 → 时序 → 起点与覆盖", 4,
               "冷启动失败、专家前缀成功；再用坐标 / 时序对照解释差异。", ORANGE)
    video_card(s, IMG_HANDOFF_COLD, VID_HANDOFF_COLD,
               "冷启动 · 未成功", "仅策略 · 诊断", 0.72, 1.72, 3.72, 2.46, RED, "20.6 s")
    video_card(s, IMG_HANDOFF_EXPERT, VID_HANDOFF_EXPERT,
               "专家前缀 · 成功", "前缀 36 步 · 接管成功", 4.78, 1.72, 3.72, 2.46, GREEN, "20.6 s")
    image_card(s, IMG_RETARGET_ALL, "坐标 / 释放对照", "±30 mm 可复现；释放行 85", 8.84, 1.72, 3.78, 2.46, CYAN, "诊断图")
    shape_rect(s, 0.72, 4.52, 11.90, 1.72, PANEL, 0.10, GRID, 0.7)
    add_text(s, "读法", 1.02, 4.82, 0.85, 0.22, 13.5, CYAN, True)
    add_text(s, "01", 1.02, 5.22, 0.34, 0.18, 10.5, CYAN, True, FONT_LATIN)
    add_text(s, "坐标合同先固定", 1.46, 5.20, 1.72, 0.20, 11.0, WHITE, True, FONT_CN)
    add_text(s, "02", 3.58, 5.22, 0.34, 0.18, 10.5, ORANGE, True, FONT_LATIN)
    add_text(s, "机械臂路径可保持一致，夹爪时序仍会改变结果", 4.02, 5.20, 3.44, 0.20, 10.3, WHITE, True, FONT_CN)
    add_text(s, "03", 7.78, 5.22, 0.34, 0.18, 10.5, GREEN, True, FONT_LATIN)
    add_text(s, "起点和首行覆盖决定后面能不能接上", 8.22, 5.20, 3.46, 0.20, 10.8, WHITE, True, FONT_CN)
    add_text(s, "StackCube 记录：第 37 步夹爪已闭合，但 TCP 离方块约 25 cm；400 步内未抓取。", 1.02, 5.72, 10.98, 0.22, 10.3, RED, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "artifacts/libero_causal_ab_20260910/ · artifacts/libero_timing_20260911/ · artifacts/libero_release_first_20260913/", "定位问题")

    # Page 5 — CALVIN has enough distinct clips to show both sides cleanly.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "CALVIN", "单任务有成功，多任务仍停在中途", 5,
               "单任务回放先看完成样例，多任务回放再看接力停点。", ORANGE)
    shape_rect(s, 0.72, 1.62, 11.90, 2.40, PANEL, 0.10, GRID, 0.7)
    pill(s, "单任务 · 已完成", 0.98, 1.80, 1.54, GREEN, NAVY, 8.2)
    video_card(s, IMG_SERVER_DRAWER_SEQ09, VID_SERVER_DRAWER_SEQ09,
               "开抽屉 · 成功", "sequence 09", 0.98, 2.10, 3.40, 1.72, GREEN, "6.8 s")
    video_card(s, IMG_CALVIN_PUSH_POSTER, VID_CALVIN_PUSH,
               "推蓝块 · 成功", "73 步", 4.62, 2.10, 3.40, 1.72, GREEN, "6.7 s")
    video_card(s, IMG_SERVER_SLIDER_SEQ05, VID_SERVER_SLIDER_SEQ05,
               "推滑门 · 成功", "sequence 05", 8.26, 2.10, 3.40, 1.72, GREEN, "14.6 s")
    shape_rect(s, 0.72, 4.28, 11.90, 2.40, PANEL_ALT, 0.10, GRID, 0.7)
    pill(s, "多任务 · 未完成", 0.98, 4.46, 1.54, RED, NAVY, 8.0)
    video_card(s, IMG_SERVER_MULTI_SEQ05_360, VID_SERVER_MULTI_SEQ05_360,
               "seq05 · 第 2/5 个任务", "第 350/360 步 · 尚未完成", 0.98, 4.76, 5.54, 1.72, ORANGE, "37.1 s")
    video_card(s, IMG_SERVER_MULTI_SEQ07_360, VID_SERVER_MULTI_SEQ07_360,
               "seq07 · 第 2/5 个任务", "第 350/360 步 · 尚未完成", 6.82, 4.76, 5.54, 1.72, ORANGE, "37.1 s")
    add_text(s, "单任务已有完成样例；多任务接力仍需形成完整完成证据。", 0.98, 6.84, 11.36, 0.16, 9.8, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "artifacts/server_media/ · new_logs/current/calvin/closed_loop_videos/", "内嵌视频 · 按结果分组")

    # Page 6 — the new residual-SAC schematic.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "强化学习接入", "在冻结 ClearVLA 外加一层动作修正", 6,
               "保留现有 G/S/W/P 两遍采样，强化学习在最终原生动作后学习小幅修正。", GREEN)
    node(s, "观测 + 指令 + 历史", "RGB/DINO · 状态\n实际执行动作", 0.72, 2.10, 2.20, 1.10, PANEL, CYAN, 12.2, 9.0)
    node(s, "冻结 ClearVLA base", "已有 G/S/W/P\n提案 → 细化", 3.42, 1.92, 2.52, 1.46, PANEL_ALT, ORANGE, 12.2, 9.0)
    node(s, "a_base", "原生动作首行", 6.42, 2.10, 1.34, 1.10, PANEL, CYAN, 11.2, 8.4)
    node(s, "残差策略（SAC）", "Soft Actor-Critic\n单位 u → Δa\n有界的小幅修正", 6.42, 3.58, 1.86, 1.28, PANEL_ALT, GREEN, 10.2, 8.2)
    node(s, "clip(a_base + Δa)", "执行命令", 8.84, 2.36, 2.16, 1.18, PANEL_ALT, GREEN, 12.0, 9.0)
    node(s, "环境", "ManiSkill\n奖励 / 成功", 11.46, 2.10, 1.18, 1.92, PANEL, ORANGE, 11.2, 8.6)
    node(s, "双 Q 评价器", "Q1 / Q2 ← 奖励\n回放转移样本", 8.84, 4.66, 2.16, 0.92, PANEL, ORANGE, 11.2, 8.4)
    draw_arrow(s, 2.94, 2.65, 3.34, 2.65, GRID, 1.0)
    draw_arrow(s, 5.98, 2.65, 6.32, 2.65, GRID, 1.0)
    draw_arrow(s, 7.82, 2.65, 8.74, 2.65, GRID, 1.0)
    draw_arrow(s, 7.88, 4.16, 8.74, 3.14, GREEN, 1.0)
    draw_arrow(s, 11.08, 2.95, 11.36, 2.95, GRID, 1.0)
    draw_arrow(s, 12.00, 4.04, 10.84, 4.56, ORANGE, 0.9)
    draw_arrow(s, 11.36, 3.56, 2.10, 5.62, GRID, 0.8)
    add_text(s, "实际执行动作回到下一轮历史", 3.12, 5.42, 4.30, 0.20, 10.5, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    shape_rect(s, 0.72, 6.08, 11.90, 0.62, PANEL_ALT, 0.08, GRID, 0.6)
    add_text(s, "代码层面：runner、replay、双 Q、checkpoint 已接入；现有闭环视频对应 base，残差仍为 0。", 0.98, 6.28, 11.36, 0.20, 10.8, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "clearvla/rl/README.md · clearvla/rl/runner.py · clearvla/rl/train.py", "接口已接入 · 效果待测")

    # Page 7 — only a local network boundary, not a second full architecture.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "局部结构", "主干已有，下一步只查语言和模块归属", 7,
               "完整 G/S/W/P 图沿用现有版本；这里只放大这轮要核对的局部路径。", CYAN)
    node(s, "T5 + state/history\n+ ObjectFactSet", "任务指令、状态和对象事实", 0.78, 2.20, 2.42, 1.18, PANEL, CYAN, 11.6, 8.8)
    node(s, "S", "语言 / 意图归属\n公共阶段载体", 3.68, 2.20, 2.10, 1.18, PANEL_ALT, ORANGE, 12.5, 8.8)
    node(s, "W1 / W2", "世界状态\n阶段 0/1 → 2/3", 6.28, 2.20, 2.12, 1.18, PANEL, GREEN, 12.2, 8.8)
    node(s, "P1 / P2", "精度 + 后果\n再解码动作", 8.90, 2.20, 2.18, 1.18, PANEL_ALT, ORANGE, 12.2, 8.8)
    node(s, "执行", "原生 7D 动作", 11.54, 2.20, 1.08, 1.18, PANEL, GREEN, 11.2, 8.4)
    draw_arrow(s, 3.26, 2.79, 3.58, 2.79, GRID, 1.0)
    draw_arrow(s, 5.86, 2.79, 6.18, 2.79, GRID, 1.0)
    draw_arrow(s, 8.46, 2.79, 8.80, 2.79, GRID, 1.0)
    draw_arrow(s, 11.18, 2.79, 11.46, 2.79, GRID, 1.0)
    shape_rect(s, 0.78, 4.10, 11.84, 1.78, PANEL, 0.10, GRID, 0.7)
    add_text(s, "这轮只核对四个边界", 1.08, 4.40, 2.34, 0.22, 13.2, CYAN, True)
    local_checks = [
        ("语言 / 意图", "S 唯一承载意图", ORANGE),
        ("有效性", "进入 S/W 的读取与注意力", RED),
        ("夹爪字段", "与部署解码对齐", GREEN),
        ("G3 → P1/P2", "只做边界 trace", CYAN),
    ]
    for i, (lab, body, col) in enumerate(local_checks):
        xx = 1.08 + (i % 2) * 5.62
        yy = 4.86 + (i // 2) * 0.52
        shape_oval(s, xx, yy + 0.03, 0.12, 0.12, col)
        add_text(s, lab, xx + 0.24, yy - 0.02, 1.46, 0.18, 10.4, col, True, FONT_CN)
        add_text(s, body, xx + 1.72, yy - 0.02, 3.52, 0.18, 10.2, WHITE, True, FONT_CN)
    add_text(s, "这里不重画整网；只把局部核对结果写回主干。", 1.08, 5.64, 10.78, 0.18, 9.8, MUTED, False, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "docs/research/00_CURRENT_ARCHITECTURE_CONTRACT.md", "局部边界 · 沿用主干")

    # Page 8 — structural plan from the current mainline repair order.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "结构修改计划", "先做边界证据，再整理模块归属", 8,
               "把语言 / S 绑定放进局部结构检查，沿同一条主线推进。", ORANGE)
    add_line(s, 1.18, 2.00, 1.18, 6.10, GRID, 2.0)
    plan_rows = [
        ("01", "先修一个确定的问题", "让 ffn_expansion 真正进入当前 decoder；核对宽度、参数量和 forward shape", CYAN),
        ("02", "把边界 trace 跑出来", "逐条核对：读取→归属→RoleDelta；夹爪→解码；G3→P1/P2；validity→未来长度", ORANGE),
        ("03", "拆开语义和归属", "语言、outlet、数据身份、恢复身份各自归属，不再共用一个字段", GREEN),
        ("04", "最后移走兼容转换", "最终模块直接构造；旧 key/order translation 留在兼容读取层", WHITE),
    ]
    for i, (n, title, body, col) in enumerate(plan_rows):
        yy = 1.86 + i * 1.08
        shape_oval(s, 0.94, yy, 0.48, 0.48, col)
        add_text(s, n, 0.94, yy + 0.13, 0.48, 0.16, 9.0, NAVY, True, FONT_LATIN, PP_ALIGN.CENTER)
        add_text(s, title, 1.76, yy - 0.01, 2.28, 0.22, 13.0, col, True, FONT_CN)
        add_text(s, body, 4.02, yy - 0.01, 5.56, 0.52, 10.1, WHITE, False, FONT_CN)
        if i < len(plan_rows) - 1:
            draw_arrow(s, 1.18, yy + 0.54, 1.18, yy + 0.96, GRID, 0.9)
    shape_rect(s, 9.92, 1.86, 2.70, 4.28, PANEL_ALT, 0.10, GRID, 0.7)
    add_text(s, "本轮保持不变", 10.24, 2.18, 2.00, 0.22, 13.2, RED, True)
    for i, txt in enumerate(["增益 / 配额", "硬掩码", "损失权重", "额外 ODE 轮次"]):
        bullet(s, txt, 10.24, 2.72 + i * 0.55, 2.00, size=11.2, accent=RED, h=0.24)
    add_text(s, "每个源单元只回答一个预先写清楚的问题。", 10.24, 5.28, 2.08, 0.50, 10.3, WHITE, True, FONT_CN)
    add_footer(s, "docs/research/CURRENT_MAINLINE_REPAIR_PLAN.md", "行为保持 · 局部修改")

    # Page 9 — language/S binding and the single-task → multi-task order.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "扩展顺序", "语言与任务扩展：先单任务，再多任务", 9,
               "先把语言与 S 的绑定做清楚；单任务闭环通过后再进入多任务。", GREEN)
    # A single timeline keeps the order visible without turning each step into
    # a separate marketing card.
    shape_rect(s, 0.72, 1.82, 11.90, 3.74, PANEL, 0.10, GRID, 0.7)
    steps = [
        ("01", "语言 / S 绑定", "固定任务指令和语言条件；\nS 统一承载意图，其他模块按需读取", "先验证绑定", CYAN, 2.20),
        ("02", "单任务多随机种子", "先与 base 对齐，再比较残差适配器；\n环境和随机种子保持一致", "先闭环再比较", ORANGE, 6.67),
        ("03", "多任务接力", "单任务闭环稳定后，再引入\n任务混合 / 迁移", "最后看泛化", GREEN, 11.10),
    ]
    add_line(s, 2.20, 2.46, 11.10, 2.46, GRID, 2.0)
    for i, (n, title, body, tag, col, cx) in enumerate(steps):
        shape_oval(s, cx - 0.26, 2.20, 0.52, 0.52, col)
        add_text(s, n, cx - 0.26, 2.36, 0.52, 0.16, 9.0, NAVY, True, FONT_LATIN, PP_ALIGN.CENTER)
        if i < len(steps) - 1:
            draw_arrow(s, cx + 0.32, 2.46, steps[i + 1][-1] - 0.34, 2.46, GRID, 0.9)
        add_text(s, title, cx - 1.50, 3.08, 3.00, 0.24, 12.8, col, True, FONT_CN, PP_ALIGN.CENTER)
        add_text(s, body, cx - 1.50, 3.52, 3.00, 0.94, 10.3, WHITE, False, FONT_CN, PP_ALIGN.CENTER)
        add_text(s, tag, cx - 1.50, 4.82, 3.00, 0.18, 9.6, MUTED, True, FONT_CN, PP_ALIGN.CENTER)
    shape_rect(s, 0.72, 5.72, 11.90, 0.72, PANEL, 0.09, GRID, 0.7)
    add_text(s, "强化学习作为独立试验：冻结同任务 base，依次比较基线、残差适配器和同一组随机种子。", 0.98, 5.96, 11.36, 0.22, 11.3, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "clearvla/rl/README.md · docs/research/CURRENT_MAINLINE_REPAIR_PLAN.md", "单任务 → 多任务")

    # Page 10 — a compact evidence boundary.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "当前边界", "现在能明确说到哪里", 10,
               "把已核实、未完成和正在准备的内容放在同一条证据线上。")
    boundary = [
        ("已核实", "环境和回放链可复查；CALVIN 有单任务成功片段。", IMG_SERVER_DRAWER_E4_245, GREEN),
        ("未完成", "LIBERO / StackCube 自动闭环尚未成功；CALVIN 多任务仍在运行。", IMG_STACK_200, RED),
        ("正在准备", "残差 SAC 接口和局部结构 trace 已有代码入口；强化学习效果尚待测试。", IMG_RETARGET_ALL, ORANGE),
    ]
    for i, (title, body, img, col) in enumerate(boundary):
        x = 0.72 + i * 4.06
        shape_rect(s, x, 1.82, 3.74, 3.90, PANEL_ALT if i == 1 else PANEL, 0.10, GRID, 0.7)
        add_image_cover(s, img, x + 0.14, 1.98, 3.46, 1.48, frame=True)
        pill(s, title, x + 0.28, 2.14, 1.06, col, NAVY, 8.4)
        add_text(s, body, x + 0.24, 3.78, 3.18, 0.92, 11.0, WHITE, True, FONT_CN)
        add_text(s, ["245 / 73 步成功示例", "180 / 400 步未完成", "接口已接入 · 残差为 0"][i], x + 0.24, 5.04, 3.18, 0.20, 9.5, col, True, FONT_CN)
    shape_rect(s, 0.72, 5.98, 11.90, 0.58, PANEL, 0.08, GRID, 0.6)
    add_text(s, "下一步的顺序不变：先把单任务闭环做成可复查证据，再扩大到多任务。", 0.98, 6.17, 11.36, 0.20, 11.4, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "workspace evidence + current architecture contract", "事实边界")

    # Page 11 — keep the quantitative training material together as appendix.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "附录 / 训练支撑", "训练曲线与释放动作边界", 11,
               "曲线用于核对训练路径，释放行用于核对部署首行。", ORANGE)
    shape_rect(s, 0.72, 1.74, 6.16, 4.72, PANEL, 0.10, GRID, 0.7)
    formal_rows = load_formal_epoch_rows(FORMAL_METRICS_JSONL)
    # These are the four validation summaries from the formal run, not
    # illustrative points.  Keep a small fallback so the deck can still be
    # regenerated on a checkout where the run directory is absent.
    if formal_rows:
        proposal_rmse = [num(row.get("validation", {}).get("validation_proposal_primary_rmse_physical")) for row in formal_rows]
        value_corr = [num(row.get("validation", {}).get("loss_execution_value_correlation")) for row in formal_rows]
        event_ratio = [num(row.get("validation", {}).get("validation_decoded_gripper_event_ratio")) for row in formal_rows]
    else:
        proposal_rmse = [0.6463, 0.5201, 0.3737, 0.2979]
        value_corr = [0.3741, 0.6340, 0.5950, 0.5232]
        event_ratio = [33.2, 32.9, 30.8, 28.2]
    # Ignore any later non-validation rows if a longer run is appended.  The
    # page is deliberately about the four-epoch formal checkpoint shown here.
    proposal_rmse = proposal_rmse[:4]
    value_corr = value_corr[:4]
    event_ratio = event_ratio[:4]
    add_text(s, "正式训练 · 4 轮验证", 1.02, 2.04, 2.80, 0.22, 14, CYAN, True, FONT_CN)
    add_text(s, "验证指标（各自归一化）", 4.22, 2.06, 2.08, 0.18, 8.8, DIM, False, FONT_CN, PP_ALIGN.RIGHT)
    add_text(s, "提案 RMSE", 1.02, 2.48, 1.10, 0.18, 10.0, MUTED, True, FONT_CN)
    sparkline(s, proposal_rmse, 1.68, 2.44, 4.50, 1.10, CYAN)
    add_text(s, "价值相关", 1.02, 3.92, 1.10, 0.18, 10.0, MUTED, True, FONT_CN)
    sparkline(s, value_corr, 1.68, 3.88, 4.50, 1.10, ORANGE)
    add_text(s, "epoch 1", 1.68, 5.05, 0.70, 0.16, 8.2, DIM, False, FONT_LATIN)
    add_text(s, "epoch 4", 5.48, 5.05, 0.70, 0.16, 8.2, DIM, False, FONT_LATIN, PP_ALIGN.RIGHT)
    metric_card(s, "提案 RMSE", f"{proposal_rmse[-1]:.3f}", 1.02, 5.40, 1.54, GREEN,
                f"{proposal_rmse[0]:.3f}→{proposal_rmse[-1]:.3f}", 0.72)
    metric_card(s, "价值相关", f"{value_corr[-1]:.3f}", 2.74, 5.40, 1.54, CYAN,
                f"{value_corr[0]:.3f}→{value_corr[-1]:.3f}", 0.72)
    metric_card(s, "事件比", f"{event_ratio[-1]:.1f}×", 4.46, 5.40, 1.54, RED,
                f"{event_ratio[0]:.1f}→{event_ratio[-1]:.1f}", 0.72)
    shape_rect(s, 7.18, 1.74, 5.44, 4.72, PANEL_ALT, 0.10, GRID, 0.7)
    add_text(s, "释放动作 / 首行边界", 7.52, 2.04, 3.62, 0.22, 14, ORANGE, True, FONT_CN)
    add_text(s, "release audit · 部署侧", 10.04, 2.08, 2.22, 0.16, 8.5, DIM, False, FONT_CN, PP_ALIGN.RIGHT)
    add_image_contain(s, IMG_RETARGET_ALL, 7.52, 2.48, 4.74, 1.56, frame=True)
    add_text(s, "释放行 85", 7.52, 4.34, 1.46, 0.20, 11.5, ORANGE, True, FONT_CN)
    add_text(s, "+0.2265  →  −0.4698", 9.10, 4.28, 2.64, 0.28, 18.0, GREEN, True, FONT_LATIN, PP_ALIGN.RIGHT)
    add_text(s, "用途：让训练端事件与部署首行对上，再回到单任务完整闭环。", 7.52, 5.06, 4.56, 0.54, 11.0, WHITE, True, FONT_CN)
    add_footer(s, "artifacts/libero_release_first_20260913/ · runs/libero_spatial_task0_bringup_20260908/formal", "附录 · 诊断支撑")

    # Page 12 — evidence index.
    s = prs.slides.add_slide(BLANK)
    add_header(s, "附录 / 素材索引", "每个判断都能回到一组素材", 12,
               "主线保留结论，详细素材按入口归档。")
    index_rows = [
        ("环境回放", "runs/libero_spatial_task0_bringup_20260908/", IMG_LIBERO_STILL, CYAN),
        ("LIBERO 对照", "artifacts/libero_causal_ab_20260910/\nartifacts/libero_timing_20260911/", IMG_HANDOFF_COLD, ORANGE),
        ("CALVIN 视频", "artifacts/server_media/\nnew_logs/current/calvin/closed_loop_videos/", IMG_SERVER_DRAWER_SEQ09, GREEN),
        ("StackCube / 强化学习", "runs/stackcube_v2_eval_20260910/\nclearvla/rl/", IMG_STACK_200, RED),
        ("结构计划", "docs/research/00_CURRENT_ARCHITECTURE_CONTRACT.md\ndocs/research/CURRENT_MAINLINE_REPAIR_PLAN.md", IMG_RETARGET_ALL, WHITE),
    ]
    for i, (label_txt, path_txt, img, col) in enumerate(index_rows):
        yy = 1.62 + i * 0.94
        shape_rect(s, 0.72, yy, 8.20, 0.76, PANEL_ALT if i % 2 else PANEL, 0.08, GRID, 0.7)
        shape_rect(s, 0.72, yy, 0.08, 0.76, col)
        add_text(s, label_txt, 1.02, yy + 0.18, 1.72, 0.20, 11.8, col, True, FONT_CN)
        add_text(s, path_txt, 2.92, yy + 0.12, 5.58, 0.44, 9.0, WHITE, False, FONT_LATIN)
        add_image_cover(s, img, 9.26, yy + 0.04, 1.96, 0.68, frame=True)
        pill(s, ["回放", "A/B", "视频", "RL", "计划"][i], 11.42, yy + 0.24, 0.86, col, NAVY, 8.0)
    shape_rect(s, 0.72, 6.52, 11.90, 0.28, PANEL, 0.08, GRID, 0.6)
    add_text(s, "PPT 内嵌代表片段；服务器原始 MP4、episode JSON、metrics 和审计报告按索引路径保存。", 0.98, 6.59, 11.36, 0.14, 10.2, WHITE, True, FONT_CN, PP_ALIGN.CENTER)
    add_footer(s, "workspace evidence / runbooks", "appendix")

    coherent_out = ROOT / "artifacts" / "clearvla_progress_review_20260913_coherent_mainline.pptx"
    coherent_out.parent.mkdir(parents=True, exist_ok=True)
    prs.save(coherent_out)
    try:
        shutil.rmtree(_POSTER_DIR)
    except Exception:
        pass
    print(str(coherent_out))
    raise SystemExit
