...
| `codex/pen-geometry-gripper-repair-20260907` | Inherits the deferred raw-only Q5/P2 experiment, then couples geometry ingress and six-channel gripper decoding; do not merge wholesale or infer either intervention independently.  Its final `7a5c20d` does expose a separable config gap: non-default `BottomConfig.ffn_expansion` is currently ignored by `_build_decoder_config` (`3.0` still resolves to the V120 default `2.0`), while every committed preset remains `2.0`.  This has no current-default result impact and crosses the network-construction boundary, so defer it for a focused config/manifest/checkpoint unit instead of admitting it from the experiment branch. |
