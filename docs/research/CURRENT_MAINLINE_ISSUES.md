...
| HDF5 episode admission / lazy reader | The current pread path resolves manifest-selected HDF5 identities directly, but `load_episodes` still opens and materializes every selected action/state array before the first DataLoader batch. The new GPU4 B8 smoke is left untouched as a cold-start observation; its external-disk wait is not a model failure. | No behavior or throughput claim is accepted yet. The remaining question is whether an immutable index plus worker-local lazy reads can preserve episode/frame/camera order, normalizers, masks, bytes and RNG continuation. | New loader issue; no training launch |
