...
| HDF5 episode admission / lazy reader | The current pread path resolves manifest-selected HDF5 identities directly, but `load_episodes` still opens and materializes every selected action/state array before the first DataLoader batch. The opt-in index prototype now has a dataset-agnostic source/row contract, explicit `.h5`/`.hdf5` identity and stale-source checks. A progressive prototype avoids opening the full manifest and admits requested episode metadata on demand. | Real 32-episode action/state/action-state bytes, identity order and serialized z-score normalizer digests match the eager path exactly. Progressive full-manifest constructor was 28.7 ms and a fixed 32-episode sample matched rows in 173 ms, but full E1 terminal/overlay/worker/RNG parity and end-to-end training throughput remain open. | New loader issue; no training launch |
...
| 2A — loader issue; no training launch | Can HDF5 episode admission be indexed and made lazy without changing behavior? | The current path still materializes all manifest-selected action/state arrays before the first batch. The proposed `hdf5_indexed_lazy_v1` unit has no accepted implementation or speedup evidence yet. | Build an immutable index and run eager/lazy loader-only byte, identity, normalizer and RNG continuation checks, then compare cold/first-batch/warm throughput. Keep eager as default until all gates pass. |
