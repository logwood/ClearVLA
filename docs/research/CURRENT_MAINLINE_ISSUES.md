...
| HDF5 episode admission / lazy reader | The current pread path resolves manifest-selected HDF5 identities directly, but `load_episodes` still opens and materializes every selected action/state array before the first DataLoader batch. The opt-in index prototype now has a dataset-agnostic source/row contract, explicit `.h5`/`.hdf5` identity and stale-source checks. | Real 32-episode action/state/action-state bytes, identity order and serialized z-score normalizer digests match the eager path exactly. Warm selected-row timing is promising, but cold index build and full E1 worker/RNG/terminal-overlay admission are not yet accepted as a training-speed claim. | New loader issue; no training launch |
