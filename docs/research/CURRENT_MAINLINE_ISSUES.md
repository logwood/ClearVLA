...
| Priority | Question | Latest observation | Source-entry condition |
|---|---|---|---|
...
| P0 | What limits Pen far-horizon and gripper-event behavior? | At matched E6 the combined Schema32 candidate changes full/arm/gripper RMSE from 0.07315 / 0.05615 / 0.13616 to 0.07680 / 0.05789 / 0.14555, decoded F1 from 0.3694 to 0.3532 and event ratio from 0.444 to 0.406. First and band 1-4 improve slightly, but bands 5-12 and 13-24 regress. | Do not promote or tune the combined candidate. A later intervention must isolate one owner, improve far/event closure without worsening near or arm behavior, and show deployed-action responsibility. |
| P0 | Does the CALVIN outlet repair close the real task? | Shared codec is restored, but converter-v1 used `terminal-48` centers for a 24-row policy target, omitting the final 24 real actions; it also split annotation rows rather than `ep_start_end_ids.npy` trajectories. | Converter-v2 data/caches must prove terminal coverage and trajectory disjointness before a fresh formal curve, causal replay and real open_drawer rollout. |
| P1 | Can data provenance and compatibility be read independently? | Metadata-only physical-chart changes alter the composite inventory digest. | Serialize separate raw inventory, split, profile metadata, normalizer/cache/language and continuation/ABI records. |
| P1 | Are S/W interval semantics under-used in a behavior-limiting way? | Pen recovery E8 public-S variation is 0.16284, typed semantic/appearance/geometry variation 0.01730 / 0.01334 / 0.00959; W variation is 0.07028 versus Teacher 0.12152. | Behavior or a matched intervention must tie the under-use to far/gripper error before changing chronology, bandwidth or loss. |
| P1 | Does geometry address or value have stable action utility? | At matched E6, RMS-floor raises W geometry common/interval contribution from 0.2095 / 0.0589 to 0.3311 / 0.0935, while P2 geometry effect falls from 0.02116 to 0.01648 and the combined geometry-zero far-gripper action delta falls from 0.00481 to 0.00432 at 8.94% coverage. Larger W ingress therefore did not create greater P2 or action utility. | If revisited, use a geometry-only matched unit and trace W ingress -> W prediction -> P2 selection -> deployed action. Do not add another gain merely because an upstream RMS is small. |
| P1 | Has outer action/world closure regressed? | Recovery E8 proposal/refined/final-interval mismatch is 0.02863 / 0.03212 / 0.01556, close to Schema28 and far below rejected Schema29. | Reopen only if a later comparable run or behavior failure shows renewed growth. |
| P1 | Does the S typed common/residual view provide independent reverse responsibility at P2? | `intent.py::_interval_common_residual` constructs `common=mean(source)` and `residual=source-common`; `compiler.py` reconstructs the exact route. The local route-sum VJP is exactly zero for common and nonzero for residual. A matched detached-lane control preserves the exact forward route while moving the VJP to common, so this is a decomposition property rather than a broken forward edge. | Treat the fields as lossless coordinates unless a future design explicitly requires two trainable owners. Before changing them, prove an action-level benefit and decide whether the current gradient metrics should be renamed rather than adding a gain, detach, quota or forced specialization. |
| P1 | Does dynamic P1 precision lose useful basis detail at its two consumers? | Source trace confirms one tensor is intentionally consumed twice: full `[B,24,Q,H]` by ControlledTransition and a separate no-null basis read plus fixed `0.25` ingress by terminal RoleDelta; both paths have nonzero owner/VJP. The matched fixture's basis posterior is effectively uniform (`entropy 0.9999997`, max mass `0.2503`), so an equal-RMS basis-antisymmetric injection is attenuated to about `3.4e-5` at RoleDelta versus `2.5e-2` for a constant injection. The actual dynamic residual in the same fixture is already nearly basis-common (centered ratio `0.012`), so this proves a bounded cancellation risk, not a current learned-data defect. | Obtain the same basis-variation and dual-consumer trace on a real checkpoint before splitting the reader or changing its gain/entropy. Do not promote the synthetic antisymmetric injection to a structural repair. |
| P1 | Does producer-owned object validity actually bound every downstream K read? | **Closed at the source boundary.** The matched pre-repair trace found invalid-K VJP/leakage at S and W. S interval/coarse reads now receive the producer mask through `ActionIntentDock`; generic and typed W object-axis blocks mask before normalization/reduction and return finite zero for an all-invalid row. The focused intervention gives invariant valid outputs and exactly zero invalid-row VJP; the real Pen logs still show validity near 1.0, so prevalence is an open behavior observation rather than an implementation gap. | Keep the support-mask and all-invalid gates in regression. Do not replace them with a learned validity gate, route quota, entropy target or extra W pass. |
...
| P2 | Does the bottom typed-P2 router need route selection? | Five route masses remain approximately uniform while the routed value update is nonzero. | An action-level intervention must show that equal mixture, rather than the values themselves, limits behavior. |
...
without suppressing first-step, far-horizon or gripper-event detail. The
serialized run context and
[auxiliary/SCHEMA31_BSPINE_EARLY_AUDIT.md](auxiliary/SCHEMA31_BSPINE_EARLY_AUDIT.md)
own experiment identity and checkpoint observations; the repair plan owns any
future disposition. Do not infer status from the Schema31 label.
