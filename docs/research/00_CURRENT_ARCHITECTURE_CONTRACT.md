...
controller and complete 18-D output ABI. The v2 language/structure bridge is
fixed in the isolated release commit `11ea215`; its structural, permutation,
invalid-row, VJP and full-policy smoke gates pass. No benchmark behavior claim
is attached until a fresh multitask CALVIN run is trained and evaluated; old
checkpoints and the rejected v1 sidecar are not resumed or migrated. Live
process state remains in the rolling handoff.
...
12. CALVIN's opt-in `calvin_primary_object_binding_v2` is an outlet-scoped
    ...
    and is the only new CALVIN condition read by coarse action. Its supervised
    output is a K-invariant semantic role distribution (red/blue/pink plus
    null) formed from a shared, language-free visual role scorer and the
    language-conditioned pointer. It does not replace the K-object scene
    memory, enter W/P2 directly, add a slot-index embedding, or construct any
    parameter for Pen/RDT/LIBERO. Invalid slots have zero pointer/joint mass and
    an all-invalid row is exact null with zero selected context. The role target
    is a training-only sidecar keyed by source episode and causal center;
    privileged scene observations never enter OnlinePolicyInput, Teacher, W/P2
    or deployment. The rejected v1 fixed-slot sidecar is not loadable.
    `open_drawer` is a separate handle/drawer role and is not covered by the
    color-block binding claim.
...
| `ActionIntentDock` | public S context plus optional CALVIN selected `[B,1,H]` context; no typed fact re-entry, pointer/geometry diagnostic, or direct W/P2 pointer |
...
| `TrainingTargetStage` | `teacher_supports`, `build_training_targets` and history-proposal target construction | training-only detached Teacher, named target bundle and optional CALVIN sidecar role target |
...
for the baseline outlet. The opt-in `calvin_primary_object_binding_v2`
component adds one language-conditioned pointer/context route at the S→coarse
seam and a language-free, K-shared visual role scorer. Its training target is a
separate `clearvla-calvin-object-binding-v2` NPZ sidecar with episode/center
keys, a semantic role distribution (red/blue/pink plus explicit null),
coverage, ambiguity, a sorted episode-inventory SHA-256 and the split-manifest
file SHA-256. No sidecar field assigns a colour to a K row; the rejected v1
fixed-slot artifact is refused by schema validation. The loader validates every
model-facing window before materialization and rejects missing or duplicate
joins. Sidecar provenance is serialized independently from raw HDF5/DINO
cache identity; its content digest is included in the exact-resume dataset
identity while its relocatable path is not. The role branch uses only shared
visual facts and detached validity support; selected context is the only
binding output entering coarse action, then the ordinary
PhysicalActionCondition→W path. Deployment and
closed-loop evaluation do not read the sidecar or privileged simulator
`scene_obs`. Color-block tasks and `open_drawer`/handle tasks are reported as
separate panels; a color-binding result does not imply drawer-role coverage.

For the first v2 release there is one multitask training line, not a four-cell
ablation. The exact trajectory-disjoint ABC→D train split is used for fitting,
the held-out converted validation split is used for checkpoint selection and
structural sentinels, and the converted test split is diagnostic only. The
primary deployment claim is the official raw-trajectory CALVIN ABC→D
long-horizon evaluator, with color-block and drawer/handle panels reported
separately. A single-task color run is a smoke gate, never the release result.
