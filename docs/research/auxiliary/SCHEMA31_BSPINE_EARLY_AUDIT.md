...
Status: historical early-decision snapshot for the run identified below,
based on its first complete validation epoch and training through approximately
epoch 2 batch 1560. Scope clarified: 2026-09-19 UTC; no metrics were refreshed.
The continuation recommendation and E2/E4 checkpoints describe the decision
at that observation, not today's process state or pending work. Current
decisions belong in the [issue ledger](../CURRENT_MAINLINE_ISSUES.md) and
[repair plan](../CURRENT_MAINLINE_REPAIR_PLAN.md); this is not an architecture
contract or evidence for every B-spine component.
...
**Recommendation at that observation: continue the run.** The named Schema31
run had no new numerical failure or action-RMSE
