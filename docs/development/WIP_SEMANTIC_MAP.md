...
> **历史快照，非当前待办。** 本图对应下方 `e62b1b7` 工作区扫描。
> 2026-09-19 整理时，原“下一步”已由
> [当前修复计划](../research/CURRENT_MAINLINE_REPAIR_PLAN.md) 替代；实现/行为状态见
> [当前问题账本](../research/CURRENT_MAINLINE_ISSUES.md)。不要据此重开已关闭的测试或实验。
...
- `S`：当时观察到 `ffn_expansion` 到 active decoder 的映射，但尚未核验聚焦测试。后续 resolved-value 门槛已闭合，见当前修复计划；本条保留快照范围，不再产生补测任务。
- `R/C/B`：不能从本图推断新的 Pen、RDT、CALVIN、LIBERO 或 StackCube learned score；这些仍由当前 issue ledger 和对应 run context 负责。

## 6. 当时的建议路径（已由当前修复计划替代）

1. **冻结分类，不清理数据。** 先保留用户的代码和生成物，给每个路径打上 M-lane、owner、是否默认、是否有测试。
2. **当时建议先验收已写入的结构单元。** `ffn_expansion` 的 resolved-value 测试后来已补齐；其余门槛以当前计划和账本为准，不从本快照自动重跑。
