...
def test_masked_candidate_center_quarantines_invalid_nan_rows() -> None:
    values = torch.tensor(
        [[[[1.0, 2.0], [float("nan"), float("nan")], [3.0, 4.0]]]],
        requires_grad=True,
    )
    ...
    torch.testing.assert_close(values.grad[:, :, 1], torch.zeros_like(values.grad[:, :, 1]))


def test_training_camera_support_intersects_object_validity_at_the_boundary() -> None:
    ...
    support = intersect_object_camera_validity(
        object_validity,
        camera_validity,
    )
    torch.testing.assert_close(
        support,
        torch.tensor([[[[0.0], [0.0]], [[0.25], [1.0]]]]),
    )
...
def test_camera_reduction_uses_validity_times_support() -> None:
    """Camera coordinates and transport use physical validity and support."""
    ...
    weight = validity * support
    expected_coordinates = (coordinates * weight).sum(dim=2) / weight.sum(dim=2)
    expected_transport = (transport * weight).sum(dim=2) / weight.sum(dim=2)
    ...
    widened_weight = validity * widened_support
    widened_coordinates = (coordinates * widened_weight).sum(dim=2) / widened_weight.sum(dim=2)
    widened_transport = (transport * widened_weight).sum(dim=2) / widened_weight.sum(dim=2)
    torch.testing.assert_close(widened.coordinates, widened_coordinates)
    torch.testing.assert_close(widened.transport_prior, widened_transport)
    torch.testing.assert_close(
        widened.world_belief().transport_prior,
        widened_transport,
    )
    assert not torch.equal(widened.coordinates, facts.coordinates)
    assert not torch.equal(widened.transport_prior, facts.transport_prior)


def test_w_validity_is_applied_at_typed_ingress_and_camera_boundaries() -> None:
    """Object and camera validity remain continuous, zero-preserving owners."""
    ...
    # Physical validity is a typed W ingress measure.  Fractional validity
    # attenuates the owner continuously; exact-zero validity erases it.
    assert torch.count_nonzero(full_common) > 0
    assert torch.count_nonzero(full_interval) > 0
    assert torch.count_nonzero(half_common - full_common) > 0
    assert torch.count_nonzero(half_interval - full_interval) > 0
    torch.testing.assert_close(
        zero_common,
        torch.zeros_like(zero_common),
        atol=0.0,
        rtol=0.0,
    )
    torch.testing.assert_close(
        zero_interval,
        torch.zeros_like(zero_interval),
        atol=0.0,
        rtol=0.0,
    )
    ...
    assert torch.count_nonzero(half_field.transport_mean) > 0
    assert torch.count_nonzero(half_field.transport_covariance) > 0
...
def test_w_quarantines_nan_invalid_object_and_camera_rows_before_projection() -> None:
    """Producer-invalid rows cannot poison W or receive a valid-row VJP."""

    torch.manual_seed(3081)
    top = _object_top().eval()
    facts, _ = top.grounder(_local_facts(cameras=2))
    invalid = torch.ones_like(facts.validity)
    invalid[:, 1] = 0.0
    valid_rows = invalid[..., 0] > 0.0

    def poison(value: torch.Tensor) -> torch.Tensor:
        source = value.detach().clone().requires_grad_(True)
        nan_rows = torch.full_like(source, float("nan"))
        mask_shape = (
            valid_rows.shape[0],
            valid_rows.shape[1],
            *([1] * (source.ndim - 2)),
        )
        mask = valid_rows.reshape(mask_shape)
        return torch.where(mask, source, nan_rows)

    content = poison(facts.content)
    semantic = poison(facts.semantic)
    appearance = poison(facts.appearance)
    geometry = poison(facts.geometry)
    nan_camera = facts.camera_transport_prior.detach().clone()
    nan_camera[:, 1] = float("nan")
    nan_coordinates = facts.camera_coordinates.detach().clone()
    nan_coordinates[:, 1] = float("nan")
    poisoned = replace(
        facts,
        content=content,
        semantic=semantic,
        ...
        geometry=geometry,
        camera_transport_prior=nan_camera,
        camera_coordinates=nan_coordinates,
        validity=invalid,
        camera_validity=torch.ones_like(facts.camera_validity),
    )
    with torch.no_grad():
        top.dynamics.delta_head.weight.normal_(std=0.1)
    action = _physical_action_condition(torch.randn(1, 4, 7))
    base, typed_common, typed_interval, _ = top.dynamics._base(
        poisoned,
        action,
        collect_diagnostics=True,
    )
    field, _ = top.dynamics._field_with_diagnostics(
        facts=poisoned,
    ...
    assert torch.count_nonzero(typed_common[:, 1]) == 0
    assert torch.count_nonzero(typed_interval[:, :, 1]) == 0

    loss = (
        base[:, :, 0].float().square().mean()
        + field.semantic_delta[:, :, 0].float().square().mean()
    )
    content_gradient, semantic_gradient = torch.autograd.grad(
        loss,
        (content, semantic),
        allow_unused=True,
    )
    for gradient in (content_gradient, semantic_gradient):
        assert gradient is not None
        assert torch.isfinite(gradient).all()
        assert torch.count_nonzero(gradient[:, 1]) == 0
    assert torch.count_nonzero(content_gradient[:, 0]) > 0
...
def test_grounder_invalid_candidates_leave_only_finite_null_owner_mass() -> None:
    ...
    slots = torch.randn(1, grounder.objects, grounder.hidden)
    validity = torch.tensor([[[1.0], [0.0], [0.0]]])
    finite_candidates = torch.randn(1, 3, grounder.hidden)
    candidates = torch.where(
        validity.bool(),
        finite_candidates,
        torch.full_like(finite_candidates, float("nan")),
    ).requires_grad_(True)
    prior = torch.ones(1, 3, 1)
    owner_log = torch.zeros(1, 3, 1)
    owner, object_mass, null_mass, read, owner_log_probability, _ = grounder._competition(
        slots,
        candidates,
        validity,
        prior,
    ...
    assert bool(torch.isfinite(owner).all())
    ...
    torch.testing.assert_close(
        candidates.grad[:, 1:],
        torch.zeros_like(candidates.grad[:, 1:]),
    )
...
def test_teacher_quarantines_nan_invalid_dense_candidates() -> None:
    """Invalid G candidates must not turn a detached Teacher target into NaN."""

    torch.manual_seed(213)
    local = _local_facts(content=8, route=4, hidden=16)
    invalid = torch.zeros_like(local.slot_validity)
    local = replace(
        local,
        slot_validity=invalid,
        content_slots=torch.full_like(local.content_slots, float("nan")),
        semantic_slots=torch.full_like(local.semantic_slots, float("nan")),
        appearance_slots=torch.full_like(local.appearance_slots, float("nan")),
        geometry_slots=torch.full_like(local.geometry_slots, float("nan")),
        slot_coordinates=torch.full_like(local.slot_coordinates, float("nan")),
        slot_transport_prior=torch.full_like(local.slot_transport_prior, float("nan")),
        target_dino_content=torch.zeros_like(local.target_dino_content),
    )
    facts, _ = DenseObjectGrounder(
        hidden=16,
        content_dim=8,
        ...
        objects=4,
        iterations=1,
    )(local, collect_diagnostics=False)
    target, _ = ObjectFutureTeacher(content_dim=8, key_dim=4)(
        facts=facts,
        future_supports=torch.randn(1, 12, 1, 2, 2, 8),
...
def test_teacher_quarantines_nan_invalid_object_camera_rows() -> None:
    """Invalid object/camera evidence is a finite zero/null target fallback."""
    ...
    facts, _ = DenseObjectGrounder(
        ...
        objects=4,
        iterations=1,
    )(_local_facts(content=8, route=4, hidden=16), collect_diagnostics=False)
    validity = facts.validity.clone()
    validity[:, 0] = 0.0
    ...
    assert torch.isfinite(target.successor_content).all()
    assert torch.isfinite(target.transport_mean).all()
    assert torch.isfinite(target.transport_covariance).all()
    torch.testing.assert_close(
        target.semantic_delta[:, :, 0],
        torch.zeros_like(target.semantic_delta[:, :, 0]),
    )
...
def test_w_object_axis_support_isolated_forward_and_backward() -> None:
    """Invalid K rows cannot dilute, poison, or receive a valid-row VJP."""
    ...
    value = torch.randn(1, 2, 4, 16, requires_grad=True)
    masked = block(
        value,
    ...
    assert torch.count_nonzero(masked[:, :, ~support[0]]) == 0

    poisoned = value.detach().clone()
    poisoned[:, :, ~support[0]] += 1000.0
    poisoned_out = block(
        poisoned,
        causal_interval=True,
        object_support=support,
    ...
    torch.testing.assert_close(
        masked[:, :, support[0]],
        poisoned_out[:, :, support[0]],
        atol=0.0,
        rtol=0.0,
    )
    valid_loss = masked[:, :, support[0]].float().square().mean()
    value_gradient = torch.autograd.grad(valid_loss, value)[0]
    assert torch.isfinite(value_gradient).all()
    assert torch.count_nonzero(value_gradient[:, :, ~support[0]]) == 0
    assert torch.count_nonzero(value_gradient[:, :, support[0]]) > 0

    typed_value = torch.randn(1, 2, 4, 16, requires_grad=True)
    typed, _ = block(
        typed_value,
        causal_interval=True,
        typed=True,
    ...
    assert torch.isfinite(typed).all()
    assert torch.count_nonzero(typed[:, :, ~support[0]]) == 0
    typed_loss = typed[:, :, support[0]].float().square().mean()
    typed_gradient = torch.autograd.grad(typed_loss, typed_value)[0]
    assert torch.isfinite(typed_gradient).all()
    assert torch.count_nonzero(typed_gradient[:, :, ~support[0]]) == 0

    empty_support = torch.zeros(1, 4, dtype=torch.bool)
    empty = block(
        value.detach(),
        causal_interval=True,
        object_support=empty_support,
    )
    empty_typed, _ = block(
        typed_value.detach(),
        causal_interval=True,
    ...
    assert torch.isfinite(empty_typed).all()
    assert torch.count_nonzero(empty) == 0
    assert torch.count_nonzero(empty_typed) == 0


def test_w_object_support_propagates_through_w1_w2() -> None:
    """The integrated W1/W2 field keeps invalid K out of valid dynamics."""
    ...
    validity = facts.validity.clone()
    validity[:, 1] = 0.0
    facts = replace(facts, validity=validity)
    content = facts.content.detach().clone().requires_grad_(True)
    base_facts = replace(facts, content=content)
    poisoned_content = content.detach().clone()
    poisoned_content[:, 1] += 1000.0
    poisoned_facts = replace(
        facts,
        content=poisoned_content.requires_grad_(True),
    )
    action = _physical_action_condition(torch.randn(1, 4, 7))
    with torch.no_grad():
        top.dynamics.delta_head.weight.normal_(std=0.1)
    ...
    _, base_w1, _ = top.dynamics.forward_w1(
        facts=base_facts,
        action=action,
    )
    base_field, _ = top.dynamics.forward_w2(
        facts=base_facts,
        w1_state=base_w1,
    )
    _, poisoned_w1, _ = top.dynamics.forward_w1(
        facts=poisoned_facts,
        action=action,
    )
    poisoned_field, _ = top.dynamics.forward_w2(
        facts=poisoned_facts,
        w1_state=poisoned_w1,
    )
    for name in ("semantic_delta", "transport_mean", "transport_covariance"):
        left = getattr(base_field, name)[:, :, 0]
        right = getattr(poisoned_field, name)[:, :, 0]
        torch.testing.assert_close(left, right, atol=0.0, rtol=0.0)
    valid_loss = (
        base_field.semantic_delta[:, :, 0].float().square().mean()
        + base_field.transport_mean[:, :, 0].float().square().mean()
    )
    content_gradient = torch.autograd.grad(valid_loss, content)[0]
    assert torch.isfinite(content_gradient).all()
    assert torch.count_nonzero(content_gradient[:, 1]) == 0
    assert torch.count_nonzero(content_gradient[:, 0]) > 0

    empty_facts = replace(facts, validity=torch.zeros_like(facts.validity))
    _, empty_w1, _ = top.dynamics.forward_w1(
        facts=empty_facts,
        action=action,
    )
    empty_field, _ = top.dynamics.forward_w2(
        facts=empty_facts,
        w1_state=empty_w1,
    )
    assert torch.isfinite(empty_field.semantic_delta).all()
    assert torch.isfinite(empty_field.transport_mean).all()
    assert torch.isfinite(empty_field.transport_covariance).all()
...
def test_s_object_read_is_language_conditioned_and_reaches_the_coarse_action() -> None:
    """A fixed scene must not retain a language-invariant K read."""

    torch.manual_seed(3601)
    top = _object_top().eval()
    facts, _ = top.grounder(_local_facts(cameras=2))
    goal = torch.randn(1, 6, 12, requires_grad=True)
    common = {
        "goal_mask": torch.ones(1, 6, dtype=torch.bool),
        "state_history": torch.randn(1, 3, 7),
        "state": torch.randn(1, 7),
    ...
    first, _ = top.intent(goal_tokens=goal, **common)
    second, _ = top.intent(goal_tokens=-goal.detach(), **common)

    # Object facts remain language-free, but the S read over their K axis is
    # conditioned by the instruction.  This is the structural edge absent in
    # the failed CALVIN color-only design.
    assert not torch.allclose(
        first.interval_object_attention,
        second.interval_object_attention,
        atol=1.0e-6,
        rtol=1.0e-6,
    )
    first_coarse = top.coarse_action(
        first.action_dock(),
        collect_diagnostics=True,
    )
    second_coarse = top.coarse_action(
        second.action_dock(),
        collect_diagnostics=True,
    )
    assert not torch.allclose(
        first_coarse.action_prediction,
        second_coarse.action_prediction,
        atol=1.0e-6,
        rtol=1.0e-6,
    )
...
def test_s_and_coarse_object_reads_mask_invalid_k_and_all_invalid_is_finite() -> None:
    ...
    partial_validity = facts.validity.clone()
    partial_validity[:, 1] = 0.0
    partial, _ = top.intent(
        facts=replace(facts, validity=partial_validity),
        **common,
    )
    assert torch.count_nonzero(partial.object_tokens[:, 1]) == 0
    assert torch.count_nonzero(partial.interval_object_attention[..., 1]) == 0
    partial_coarse = top.coarse_action(partial.action_dock())
    assert torch.isfinite(partial_coarse.action_prediction).all()

    no_validity = torch.zeros_like(facts.validity)
    empty, _ = top.intent(
        facts=replace(facts, validity=no_validity),
        **common,
    )
    assert torch.count_nonzero(empty.object_tokens) == 0
    assert torch.count_nonzero(empty.interval_object_attention) == 0
    empty_coarse = top.coarse_action(empty.action_dock())
    assert torch.isfinite(empty.public_interval_carrier).all()
    assert torch.isfinite(empty_coarse.action_prediction).all()
...
def test_s_owns_per_type_object_relevance_and_fixed_zero_null_values() -> None:
    ...
    # Invalid object support is a zero-preserving W typed source, so it cannot
    # leave a private semantic/appearance/geometry owner behind.
    torch.testing.assert_close(common, torch.zeros_like(common), atol=0.0, rtol=0.0)
...
def test_s_lone_object_and_type_keep_fixed_owner_means_and_rms_contract() -> None:
    """S keeps fixed K/type means and one shared sqrt(3) aggregation."""
