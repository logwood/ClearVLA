...
def _log_measure(measure: torch.Tensor) -> torch.Tensor:
    ...
    return torch.where(
        ...
        torch.where(
            measure_f > 0.0,
            measure_f,
            torch.ones_like(measure_f),
        ).log(),
        torch.zeros_like(measure_f),
    )
...
def test_semantic_and_appearance_only_correct_conditional_k_mass() -> None:
    ...
    log_prior = torch.zeros(1, 1, 1)
    owner, typed_owner, mass, null, _, _, _ = grounder._competition(
        slots, content, views, validity, log_prior
    )
    ...
    for type_index in (0, 1):
        ...
        changed_owner, changed_typed, changed_mass, changed_null, _, _, _ = (
            grounder._competition(slots, content, changed, validity, log_prior)
        )
...
def test_geometry_cannot_vote_on_physical_k_identity() -> None:
    ...
    log_prior = torch.zeros(1, 3, 1)
    owner, typed_owner, mass, null, read, _, _ = grounder._competition(
        slots, content, views, validity, log_prior
    )

    changed = views.clone()
    changed[..., 2, :] = 100.0 * torch.randn_like(changed[..., 2, :])
    changed_owner, changed_typed, changed_mass, changed_null, changed_read, _, _ = (
        grounder._competition(slots, content, changed, validity, log_prior)
    )
...
def test_w2_preserves_near_rows_and_retains_same_direction_far_innovation() -> None:
    ...
    assert not near_field.semantic_delta.requires_grad
    assert not near_field.transport_mean.requires_grad
    assert working.near.requires_grad
    assert working.near_interval_innovation.requires_grad
...
def test_future_neutral_fallback_remains_supervised_when_reliability_is_zero() -> None:
    ...
    target = FutureObjectDynamics(
        ...
        log_camera_weight=torch.zeros(batch, objects, cameras, 1),
    )
    prediction = replace(
        target,
...
def test_future_transport_interval_diagnostic_uses_camera_support() -> None:
    ...
    target = FutureObjectDynamics(
        ...
        log_camera_weight=torch.full(
            (batch, objects, cameras, 1), math.log(0.5)
        ),
    )
...
def test_p2_equal_candidate_evidence_has_one_equal_type_local_null() -> None:
    ...
    semantic_null = torch.tensor(1.0 / float(intervals + 1))
    geometry_null = torch.tensor(1.0 / float(intervals + 1))
    aggregate_null = torch.tensor(1.0 / float(intervals + 1))
    torch.testing.assert_close(
        metrics["object_p2_semantic_complete_field_null_mass"],
        semantic_null,
        atol=1.0e-7,
        rtol=0.0,
    )
    torch.testing.assert_close(
        metrics["object_p2_geometry_complete_field_null_mass"],
        geometry_null,
        atol=1.0e-7,
        rtol=0.0,
    )
    torch.testing.assert_close(
        metrics["object_p2_complete_field_null_mass"],
        aggregate_null,
        atol=1.0e-7,
        rtol=0.0,
    )
    ...
    assert float(metrics["object_p2_selected_complete_field_common_rms"]) == 0.0


def test_p2_protected_common_survives_when_interval_residual_is_exactly_zero() -> None:
    """The complete W field retains common content inside interval+null."""
    ...
    dynamics = FutureObjectDynamics(
        ...
        log_camera_weight=torch.zeros(batch, objects, 1, 1),
    )
    intent = PolicyIntentDock(
        common_key=torch.zeros(batch, hidden),
        interval_residual_key=torch.zeros(batch, intervals, hidden),
...
def test_p2_real_camera_mixture_is_camera_permutation_invariant() -> None:
    ...
    dynamics = FutureObjectDynamics(
        ...
        log_camera_weight=_log_measure(camera_weights),
    )
    intent = PolicyIntentDock(
    ...
    relabeled = replace(
        ...
        log_camera_weight=dynamics.log_camera_weight[:, :, camera_permutation],
    ...
    # hypotheses. Geometry must respond and its marginalized evidence may now
    # condition semantic K identity; transport was not averaged before either
    # consumer.
    unpaired = replace(
        dynamics,
        transport_mean=dynamics.transport_mean[:, :, :, camera_permutation],
    ...
    no_camera = replace(
        ...
        log_camera_weight=torch.zeros_like(dynamics.log_camera_weight),
...
def test_p2_invalid_objects_have_exactly_zero_common_and_residual_support() -> None:
    ...
    baseline = FutureObjectDynamics(
        ...
        log_chart_availability=_log_measure(validity),
        camera_coordinates=torch.zeros(batch, objects, 1, 2),
        camera_chart_availability=torch.ones(batch, objects, 1, 1),
        camera_weights=torch.ones(batch, objects, 1, 1),
        log_camera_weight=torch.zeros(batch, objects, 1, 1),
...
def test_p2_consumer_type_names_map_to_the_matching_s_owner() -> None:
    ...
    for p2_index, s_index in enumerate((0, 2)):
        assert len(captured[p2_index]) == 1
        torch.testing.assert_close(
            captured[p2_index][0],
            (
                intent.typed_common_object_value[..., s_index, :][:, None]
...
def test_p2_semantic_intervention_cannot_change_geometry_selector() -> None:
    ...
    assert not torch.equal(
        metrics["object_p2_semantic_score_max_abs"],
        changed_metrics["object_p2_semantic_score_max_abs"],
    )
    assert not torch.equal(
        metrics["object_p3_semantic_interval_posterior_entropy"],
        changed_metrics["object_p3_semantic_interval_posterior_entropy"],
    )
    for suffix in (
        "score_max_abs",
        "spatial_posterior_entropy",
        "spatial_posterior_max",
    ):
        torch.testing.assert_close(
    ...
    for suffix in ("interval_posterior_entropy", "interval_posterior_max"):
        torch.testing.assert_close(
            metrics[f"object_p3_geometry_{suffix}"],
            changed_metrics[f"object_p3_geometry_{suffix}"],
            atol=0.0,
            rtol=0.0,
        )
...
def test_p2_matched_interval_routes_read_typed_s_and_w_object_evidence() -> None:
    ...
    baseline_mass = torch.stack(
        [
            baseline_metrics[f"object_p3_interval_{index}_mass"]
            for index in range(4)
        ]
    )
    changed_mass = torch.stack(
        [
            changed_metrics[f"object_p3_interval_{index}_mass"]
            for index in range(4)
        ]
    )
    assert not torch.equal(changed_mass, baseline_mass)
    assert float(changed_metrics["object_p3_interval_terminal_has_null"]) == 0.0


def test_p2_public_interval_prior_responds_to_public_s_interval_keys() -> None:
    """Public S conditions a nonzero W key, but is not an independent vote."""
    ...
    query = torch.randn(1, 24, 2, 32)
    baseline_dynamics = context.predicted_dynamics
    dynamics = replace(
        baseline_dynamics,
        semantic_delta=(
            baseline_dynamics.semantic_delta
            + 0.2 * torch.randn_like(baseline_dynamics.semantic_delta)
        ),
        transport_mean=(
            baseline_dynamics.transport_mean
            + 0.1 * torch.randn_like(baseline_dynamics.transport_mean)
        ),
    )
    intent = context.intent.policy_dock()
    ...
    for type_name in ("semantic", "geometry"):
        baseline_typed_mass = torch.stack(
            [
                baseline_metrics[
                        f"object_p3_{type_name}_interval_{index}_mass"
                ]
                for index in range(4)
            ]
        )
...
def test_p2_residual_retention_metrics_are_support_aware_and_complementary() -> None:
    ...
    retained = metrics["object_p3_interval_innovation_retained_rms_ratio"]
    cancelled = metrics["object_p3_interval_innovation_cancelled_rms_fraction"]
    support = metrics[
    ...
    assert float(
        neutral_metrics["object_p3_interval_innovation_retained_rms_ratio"]
    ) == 0.0
    assert float(
        neutral_metrics[
...
def test_consequence_preserves_typed_effect_and_uses_one_physical_sum() -> None:
    ...
    try:
        ...
        raise AssertionError("Schema38 consequence must reject an untyped effect")
...
def test_p3_six_optional_lanes_are_source_exclusive_and_zero_preserving() -> None:
    ...
    # The live P1 residual is a required active precision condition. It never
    # becomes part of the protected factual consequence.
    residual_parameter = inspect.signature(compiler.forward).parameters[
        "p1_policy_residual"
    ]
    assert residual_parameter.default is inspect.Parameter.empty
