...
def test_calvin_validation_removes_raw_zero_offset_from_motion_accounting() -> None:
    horizon = 24
    action_dim = 7
    offset = np.asarray(
        [[0.70, -0.30, 0.20, 0.40, -0.10, 0.50, 0.25]],
        dtype=np.float32,
    )
    scale = np.asarray(
        [[2.0, 1.5, 0.5, 3.0, 0.8, 1.2, 0.75]],
        dtype=np.float32,
    )
    normalizer = ArrayNormalizer(
        offset=offset,
    ...
    raw_target = torch.zeros(1, horizon, action_dim)
    raw_target[..., -1] = 1.0
    normalized_target = torch.as_tensor(
        normalizer.encode(raw_target.numpy()),
        dtype=torch.float32,
    )
    ...
    batch = SimpleNamespace(
        action_target=SimpleNamespace(
            ...
            gripper_transition_boundary=normalized_current,
            gripper_transition_boundary_raw_units=raw_target[:, 0],
        ),
        online=SimpleNamespace(
            history=SimpleNamespace(
                action_state=normalized_current,
                codec_gripper_boundary=normalized_current[..., -1:],
            ),
        ),
    )
    # CALVIN deployment leaves arm coordinates normalized but publishes the
    # gripper in its strict native {-1,+1} command alphabet.
    prediction = normalized_target.clone()
    prediction[..., -1] = 1.0
    command_logits = torch.zeros(1, horizon, 2)
    command_logits[..., 1] = 1.0
    accumulator = ValidationAccumulator.from_action_normalizer(
        normalizer,
        device=torch.device("cpu"),
    ...
    accumulator.update(
        prediction,
        cast(TrainingBatch, batch),
        gripper_command_logits=command_logits,
        gripper_command=torch.ones(1, horizon),
    )

    metrics = accumulator.means()
    assert metrics["validation_action_rmse_normalized"] == 0.0
    assert metrics["validation_motion_head_events_target"] == 0.0
    assert metrics["validation_motion_head_events_predicted"] == 0.0
    assert metrics["validation_motion_head_target_rate"] == 0.0
    assert metrics["validation_motion_head_predicted_rate"] == 0.0
    assert metrics["validation_motion_head_accuracy"] == 1.0
    # The binary command chart is reported separately from continuous
    # gripper trajectory RMSE so CALVIN and Pen cannot be compared by name.
    assert metrics["validation_gripper_binary_command_rmse_normalized"] == 0.0
    assert metrics["validation_gripper_binary_command_rmse_source_native"] == 0.0
    assert metrics["validation_gripper_binary_command_error_rate"] == 0.0

    wrong_prediction = prediction.clone()
    wrong_prediction[..., -1] = -1.0
    wrong_logits = torch.zeros_like(command_logits)
    wrong_logits[..., 0] = 1.0
    wrong_accumulator = ValidationAccumulator.from_action_normalizer(
        normalizer,
        device=torch.device("cpu"),
        arm_motion_threshold=0.02,
        gripper_output_mode="calvin_binary_command",
    ...
    wrong_accumulator.update(
        wrong_prediction,
        cast(TrainingBatch, batch),
        gripper_command_logits=wrong_logits,
        gripper_command=-torch.ones(1, horizon),
    )
    wrong_metrics = wrong_accumulator.means()
    assert wrong_metrics["validation_gripper_binary_command_error_rate"] == 1.0
    assert wrong_metrics[
        "validation_gripper_binary_command_rmse_source_native"
    ] == pytest.approx(2.0)
    assert wrong_metrics[
        "validation_gripper_binary_command_rmse_normalized"
    ] == pytest.approx(2.0 * float(scale[0, -1]))
    assert wrong_metrics["validation_gripper_command_rmse_physical"] == pytest.approx(
        wrong_metrics["validation_gripper_binary_command_rmse_source_native"]
    )
