...
def test_mainline_manifest_names_the_current_component_semantics() -> None:
    ...
    assert ARCHITECTURE_MANIFEST.schema == 17
    assert components.observation == "current_dino_raw_pair_current_aligned_flow_pre_g_v4"
    assert components.top == "object_intent_dynamics_323_conditional_object_writes_v11"
    assert components.bottom == "typed_read_only_physical_evidence_mmdit_controlled_transition_v6"
    assert (
        components.training
        == "single_stage_physical_action_band_event_balanced_exact_null_v9"
    )
    assert components.runtime == "minimal_cached_physical_transition_five_step_ode_v8"


def test_schema_17_active_parameter_inventory_cannot_silently_shrink() -> None:
    ...
    assert counts(model) == (171_355_774, 171_253_374)
    assert {name: counts(module) for name, module in model.named_children()} == {
        "observation": (12_857_989, 12_857_989),
        "action_codec": (0, 0),
        "top": (98_808_785, 98_710_481),
        "history_proposal": (10_014_727, 10_010_631),
        "factual_reader": (8_534_528, 8_534_528),
        "transition": (8_695_817, 8_695_817),
