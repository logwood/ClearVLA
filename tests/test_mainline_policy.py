...
def test_calvin_velocity_is_invariant_to_compatibility_gripper_field() -> None:
    ...
    model.configure_action_normalizer(
        SimpleNamespace(offset=np.zeros((1, 7)), scale=np.ones((1, 7)))
    )
...
def test_first_step_release_auxiliary_targets_only_row_zero_openings() -> None:
    ...
    config = replace(
        ...
        data=replace(
            ...
            split_mode="episode-manifest",
            split_manifest="fixture-split.json",
            train_episodes=0,
            val_episodes=0,
            test_episodes=0,
            sampling_gripper_event_threshold=0.1,
        ...
        objectives=replace(base.objectives, gripper_first_step_release=0.05),
    )
    config.validate()
    model = ClearVLAMainlinePolicy(config).train()
    batch = _batch(config, batch=2)
    ...
    assert terms["gripper_first_step_release_rows"] == 1
    assert terms["gripper_first_step_release"] > 0
    terms["gripper_first_step_release"].backward()
    assert tokens.grad is not None
    assert torch.isfinite(tokens.grad).all()


def test_stackcube_first_step_hold_auxiliary_targets_no_event_row_zero() -> None:
    ...
    config = replace(
        ...
        data=replace(
            ...
            split_mode="episode-manifest",
            split_manifest="fixture-stackcube-split.json",
            train_episodes=0,
            val_episodes=0,
            test_episodes=0,
            sampling_gripper_event_threshold=0.1,
        ...
        bottom=replace(base.bottom, arm_flow_mode="relative_command_adapter"),
        objectives=replace(base.objectives, gripper_first_step_hold=0.05),
    )
    config.validate()
    model = ClearVLAMainlinePolicy(config).train()
    batch = _batch(config, batch=2)
    ...
    # Sample 0 is a true hold at row zero followed by a later close.  Sample 1
    # opens immediately, so it must not contribute to the hold auxiliary.
    target_action[0, :, -1] = 1.0
    target_action[0, 3:, -1] = -1.0
    ...
    target = replace(
        ...
        raw_units=target_action,
        current_raw_units=action_state,
        gripper_transition_boundary=action_state,
        gripper_transition_boundary_raw_units=action_state,
    )
    target_physical = model.outlet_adapter.encode(target_action, action_state)
    zero_physical = torch.zeros_like(target_physical)
    tokens = torch.randn(2, 24, 32, requires_grad=True)
    ...
    assert terms["gripper_first_step_hold_rows"] == 1
    assert terms["gripper_first_step_hold"] > 0
    terms["gripper_first_step_hold"].backward()
    assert tokens.grad is not None
    assert torch.isfinite(tokens.grad).all()
