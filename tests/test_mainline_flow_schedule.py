...
def test_q5_schedule_delivers_bottom_numerical_context_at_each_node_and_endpoint() -> None:
    config = _config()
    plan = DeploymentFlowSchedule.same_nfe_power_five()
    model = _Model(_time_and_state_field)
    sample_cached_action(
        model,
        _cache(),
        config,
    ...
    contexts = [call["flow_step_context"] for call in model.calls]
    assert all(isinstance(value, FlowStepContext) for value in contexts)
    updates = contexts[:5]
    endpoint = contexts[5]
    expected_sizes = plan.refined.step_sizes
    for index, (context, expected_size) in enumerate(zip(updates, expected_sizes)):
        assert isinstance(context, FlowStepContext)
        torch.testing.assert_close(context.time, torch.full((1,), plan.refined.query_times[index]))
        torch.testing.assert_close(context.step_size, torch.full((1,), expected_size))
        torch.testing.assert_close(context.normalized_index, torch.full((1,), index / 5.0))
        torch.testing.assert_close(context.endpoint, torch.zeros(1))
    assert isinstance(endpoint, FlowStepContext)
    torch.testing.assert_close(endpoint.time, torch.ones(1))
    torch.testing.assert_close(endpoint.step_size, torch.zeros(1))
    torch.testing.assert_close(endpoint.normalized_index, torch.ones(1))
    torch.testing.assert_close(endpoint.endpoint, torch.ones(1))


def test_uniform_e5_keeps_the_legacy_velocity_abi_without_context() -> None:
    config = _config()
    model = _Model(_time_and_state_field)
    sample_cached_action(
        model,
    ...
    assert all(call["flow_step_context"] is None for call in model.calls)
...
def test_q5_schedule_round_trip_identity_and_invalid_nodes_fail_closed() -> None:
    ...
    assert q5.fingerprint == (
        "814c287d6cba17475d7f3d1b1d2facc8e65da3a694eff1f76256a5f319d92ac7"
    )
    assert q5.to_dict()["bottom_context_contract"] == (
        "flow_step_context_t_dt_kn_endpoint_fourier_v1"
    )
