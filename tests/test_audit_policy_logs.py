...
class AuditPolicyLogsTest(unittest.TestCase):
    ...
    def test_schema28_recovery_requires_public_private_ownership_metrics(self) -> None:
        baseline = _complete_recovery_summary("v120")
        candidate = deepcopy(baseline)
        candidate["label"] = "schema28"
        ...
        self.assertEqual(
            missing_checks["structure/object_w_object_innovation_variation"],
            "incomplete",
        )

    def test_schema29_recovery_requires_complementary_fusion_metrics(self) -> None:
        baseline = _complete_recovery_summary("v120")
        candidate = deepcopy(baseline)
        candidate["label"] = "schema29"
        candidate["manifest"]["architecture_schema"] = 29
        active_structure = (
            "object_grounding_object_content_pair_cosine",
            "object_grounding_object_innovation_pair_cosine",
            "object_grounding_prebind_typed_consensus_l1",
        ...
        candidate["structure"].pop("object_p2_fusion_residual_rms")
        missing = _recovery_assessment(baseline, candidate)
        missing_checks = {
            item["name"]: item["status"] for item in missing["checks"]
        }
        self.assertEqual(
            missing_checks["structure/object_p2_fusion_residual_rms"],
            "incomplete",
        )
    ...
    def test_schema31_recovery_requires_common_residual_chain_metrics(self) -> None:
        baseline = _complete_recovery_summary("v120")
        candidate = deepcopy(baseline)
        candidate["label"] = "schema31"
        candidate["manifest"]["architecture_schema"] = 31
        active_structure = (
            "object_grounding_reconstruction_object_mass_mean",
            "object_grounding_reconstruction_active_fraction",
            "object_grounding_reconstruction_conditional_owner_entropy",
        ...
        candidate["structure"].pop("object_p2_protected_common_rms")
        missing = _recovery_assessment(baseline, candidate)
        missing_checks = {
            item["name"]: item["status"] for item in missing["checks"]
        }
        self.assertEqual(
            missing_checks["structure/object_p2_protected_common_rms"],
            "incomplete",
        )

    def test_schema32_recovery_requires_continuous_flow_and_null_audits(self) -> None:
        baseline = _complete_recovery_summary("v120")
        candidate = deepcopy(baseline)
        candidate["label"] = "schema32"
        candidate["manifest"]["architecture_schema"] = 32
        active_structure = (
            "object_grounding_reconstruction_object_mass_mean",
            "object_grounding_reconstruction_active_fraction",
            "object_grounding_reconstruction_conditional_owner_entropy",
        ...
        candidate["structure"].pop("evidence_trajectory_summary_norm")
        missing = _recovery_assessment(baseline, candidate)
        missing_checks = {
            item["name"]: item["status"] for item in missing["checks"]
        }
        self.assertEqual(
            missing_checks["structure/evidence_trajectory_summary_norm"],
            "incomplete",
        )
    ...
    def test_schema34_recovery_requires_mapped_temporal_and_value_unit_audits(self) -> None:
        baseline = _complete_recovery_summary("v120")
        candidate = deepcopy(baseline)
        candidate["label"] = "schema34"
        candidate["manifest"]["architecture_schema"] = 34
        candidate["structure"] = {
            name: {"tail_median": 0.5 if name.endswith("pair_cosine") else 0.1}
            for name in STRUCTURE_KEYS
        }
        ...
        missing_name = "object_p2_geometry_common_value_contract_scale_mean"
