...
OUT_DIR=${OUT_DIR:-runs/v53_vertical_depth_b8}
# V53-A: shortcut suppression + diagnostics (on by default).
LATENT_CVAE_NOISY_GATE=${LATENT_CVAE_NOISY_GATE:-1}
LATENT_CVAE_NOISY_GATE_MIN=${LATENT_CVAE_NOISY_GATE_MIN:-0.05}
LATENT_CVAE_NOISY_GATE_POWER=${LATENT_CVAE_NOISY_GATE_POWER:-1.5}
LAYER_BOOST_RESIDUAL=${LAYER_BOOST_RESIDUAL:-1}
LAYER_ZERO_BASE_DIAGNOSTIC=${LAYER_ZERO_BASE_DIAGNOSTIC:-1}
# V53-B: lateral-to-vertical (on by default).
LATENT_CVAE_LAYER_SCAN=${LATENT_CVAE_LAYER_SCAN:-1}
LATENT_CVAE_LAYER_SCAN_ALPHA=${LATENT_CVAE_LAYER_SCAN_ALPHA:-0.2}
ADAPTIVE_CVAE_MONOTONIC_LAYER_ROUTE=${ADAPTIVE_CVAE_MONOTONIC_LAYER_ROUTE:-1}
ADAPTIVE_CVAE_LAYER_ROUTE_DISTANCE_SCALE=${ADAPTIVE_CVAE_LAYER_ROUTE_DISTANCE_SCALE:-3.0}
# V53-C: structural convergence (off by default; enable after A/B logs look sane).
LATENT_CVAE_CANVAS_CROSS_ATTENTION=${LATENT_CVAE_CANVAS_CROSS_ATTENTION:-0}
ADAPTIVE_CVAE_SERIAL_WRITERS=${ADAPTIVE_CVAE_SERIAL_WRITERS:-0}
# V53.1: coefficient-space unification, refine-only mid supervision,
# and the direct condition residual that replaces the removed micro
# context-modulation path (without it the capsule routed context is dead).
LATENT_CVAE_TRAJECTORY_PINV=${LATENT_CVAE_TRAJECTORY_PINV:-1}
LATENT_CVAE_TRAJECTORY_RIDGE=${LATENT_CVAE_TRAJECTORY_RIDGE:-1e-2}
LATENT_CVAE_TRAJECTORY_MID_REFINE_ONLY=${LATENT_CVAE_TRAJECTORY_MID_REFINE_ONLY:-1}
ADAPTIVE_CVAE_DIRECT_CONDITION_RESIDUAL=${ADAPTIVE_CVAE_DIRECT_CONDITION_RESIDUAL:-1}
ADAPTIVE_CVAE_CONDITION_STRENGTH=${ADAPTIVE_CVAE_CONDITION_STRENGTH:-1}
# V53.2: x_t-branch amplification hinge (counter to xratio creep).
LATENT_CVAE_NOISY_RATIO_MAX=${LATENT_CVAE_NOISY_RATIO_MAX:-0.35}
LATENT_CVAE_NOISY_RATIO_WEIGHT=${LATENT_CVAE_NOISY_RATIO_WEIGHT:-0.05}
# V53.3: hard cap on the ungated trajectory_context channel (V53.5: relaxed
# 1.0 -> 2.5 per observed healthy working range).
LATENT_CVAE_TRAJECTORY_CONTEXT_NORM_MAX=${LATENT_CVAE_TRAJECTORY_CONTEXT_NORM_MAX:-2.5}
# V53.5: v51 root-cause fixes -- positional basis exempt from trajectory
# smoothing, and the v50 per-channel Gaussian bridge back on (its removal in
# v51 both unbalanced the delta channels and silently broke eval noise).
LATENT_CVAE_TRAJECTORY_POS_EXEMPT=${LATENT_CVAE_TRAJECTORY_POS_EXEMPT:-1}
BLOCK_ACTION_DENOISE_MATRIX=${BLOCK_ACTION_DENOISE_MATRIX:-1}
...
LATENT_CVAE_ARM_COEFF_OUTPUT=${LATENT_CVAE_ARM_COEFF_OUTPUT:-0}
LATENT_CVAE_ARM_COEFF_POINTS=${LATENT_CVAE_ARM_COEFF_POINTS:-8}
LATENT_CVAE_ARM_COEFF_BASIS=${LATENT_CVAE_ARM_COEFF_BASIS:-dct}
LATENT_CVAE_ARM_COEFF_DEGREE=${LATENT_CVAE_ARM_COEFF_DEGREE:-2}
LATENT_CVAE_ARM_COEFF_RIDGE=${LATENT_CVAE_ARM_COEFF_RIDGE:-1e-2}
LATENT_CVAE_ARM_COEFF_WRITER=${LATENT_CVAE_ARM_COEFF_WRITER:-analysis}
LATENT_CVAE_COEFF_INCLUDE_GRIPPER=${LATENT_CVAE_COEFF_INCLUDE_GRIPPER:-0}
LATENT_CVAE_COEFF_MAGNITUDE_GROUPS=${LATENT_CVAE_COEFF_MAGNITUDE_GROUPS:-2}
LATENT_CVAE_SPLINE_BASE_DIAGNOSTICS=${LATENT_CVAE_SPLINE_BASE_DIAGNOSTICS:-1}
...
ADAPTIVE_CVAE_DETAIL_MICRO=${ADAPTIVE_CVAE_DETAIL_MICRO:-0}
ADAPTIVE_CVAE_DETAIL_MICRO_SUPERVISION=${ADAPTIVE_CVAE_DETAIL_MICRO_SUPERVISION:-0}
ADAPTIVE_CVAE_DETAIL_MICRO_SCALE=${ADAPTIVE_CVAE_DETAIL_MICRO_SCALE:-0.25}
ADAPTIVE_CVAE_DETAIL_MICRO_GATE_INIT=${ADAPTIVE_CVAE_DETAIL_MICRO_GATE_INIT:-0.35}
...
LATENT_CVAE_MICRO_SUPERVISION_WEIGHT=${LATENT_CVAE_MICRO_SUPERVISION_WEIGHT:-0.05}
LATENT_CVAE_MICRO_EVENT_WEIGHT=${LATENT_CVAE_MICRO_EVENT_WEIGHT:-0.01}
LATENT_CVAE_MICRO_MONOTONIC_WEIGHT=${LATENT_CVAE_MICRO_MONOTONIC_WEIGHT:-0.01}
LATENT_CVAE_MICRO_WEIGHT_KL_WEIGHT=${LATENT_CVAE_MICRO_WEIGHT_KL_WEIGHT:-0.0005}
LATENT_CVAE_MICRO_COVERAGE_SMOOTH_WEIGHT=${LATENT_CVAE_MICRO_COVERAGE_SMOOTH_WEIGHT:-0.001}
LATENT_CVAE_MICRO_COVERAGE_FLOOR_WEIGHT=${LATENT_CVAE_MICRO_COVERAGE_FLOOR_WEIGHT:-0.001}
LATENT_CVAE_MICRO_LEARNED_WEIGHT_MAX=${LATENT_CVAE_MICRO_LEARNED_WEIGHT_MAX:-0.35}
LATENT_CVAE_MICRO_LEARNED_RAMP_STEPS=${LATENT_CVAE_MICRO_LEARNED_RAMP_STEPS:-2000}
...
python -u -m clearvla.cli.train_v40_policy \
  ...
  --latent-cvae-arm-coeff-output "${LATENT_CVAE_ARM_COEFF_OUTPUT}" \
  --latent-cvae-arm-coeff-points "${LATENT_CVAE_ARM_COEFF_POINTS}" \
  --latent-cvae-arm-coeff-basis "${LATENT_CVAE_ARM_COEFF_BASIS}" \
  --latent-cvae-arm-coeff-degree "${LATENT_CVAE_ARM_COEFF_DEGREE}" \
  --latent-cvae-arm-coeff-ridge "${LATENT_CVAE_ARM_COEFF_RIDGE}" \
  --latent-cvae-arm-coeff-writer "${LATENT_CVAE_ARM_COEFF_WRITER}" \
  --latent-cvae-coeff-include-gripper "${LATENT_CVAE_COEFF_INCLUDE_GRIPPER}" \
  --latent-cvae-coeff-magnitude-groups "${LATENT_CVAE_COEFF_MAGNITUDE_GROUPS}" \
  --latent-cvae-spline-base-diagnostics "${LATENT_CVAE_SPLINE_BASE_DIAGNOSTICS}" \
  ...
  --adaptive-cvae-detail-micro "${ADAPTIVE_CVAE_DETAIL_MICRO}" \
  --adaptive-cvae-detail-micro-supervision "${ADAPTIVE_CVAE_DETAIL_MICRO_SUPERVISION}" \
  --adaptive-cvae-detail-micro-scale "${ADAPTIVE_CVAE_DETAIL_MICRO_SCALE}" \
  --adaptive-cvae-detail-micro-gate-init "${ADAPTIVE_CVAE_DETAIL_MICRO_GATE_INIT}" \
  ...
  --latent-cvae-micro-supervision-weight "${LATENT_CVAE_MICRO_SUPERVISION_WEIGHT}" \
  --latent-cvae-micro-event-weight "${LATENT_CVAE_MICRO_EVENT_WEIGHT}" \
  --latent-cvae-micro-monotonic-weight "${LATENT_CVAE_MICRO_MONOTONIC_WEIGHT}" \
  --latent-cvae-micro-weight-kl-weight "${LATENT_CVAE_MICRO_WEIGHT_KL_WEIGHT}" \
  --latent-cvae-micro-coverage-smooth-weight "${LATENT_CVAE_MICRO_COVERAGE_SMOOTH_WEIGHT}" \
  --latent-cvae-micro-coverage-floor-weight "${LATENT_CVAE_MICRO_COVERAGE_FLOOR_WEIGHT}" \
  --latent-cvae-micro-learned-weight-max "${LATENT_CVAE_MICRO_LEARNED_WEIGHT_MAX}" \
  --latent-cvae-micro-learned-ramp-steps "${LATENT_CVAE_MICRO_LEARNED_RAMP_STEPS}" \
  --weight-decay 0.01 \
  --beta1 0.9 \
  --beta2 0.999 \
  --eps 1e-8 \
  ...
  --latent-cvae-noisy-gate "${LATENT_CVAE_NOISY_GATE}" \
  --latent-cvae-noisy-gate-min "${LATENT_CVAE_NOISY_GATE_MIN}" \
  --latent-cvae-noisy-gate-power "${LATENT_CVAE_NOISY_GATE_POWER}" \
  --layer-boost-residual "${LAYER_BOOST_RESIDUAL}" \
  --layer-zero-base-diagnostic "${LAYER_ZERO_BASE_DIAGNOSTIC}" \
  --latent-cvae-layer-scan "${LATENT_CVAE_LAYER_SCAN}" \
  --latent-cvae-layer-scan-alpha "${LATENT_CVAE_LAYER_SCAN_ALPHA}" \
  --adaptive-cvae-monotonic-layer-route "${ADAPTIVE_CVAE_MONOTONIC_LAYER_ROUTE}" \
  --adaptive-cvae-layer-route-distance-scale "${ADAPTIVE_CVAE_LAYER_ROUTE_DISTANCE_SCALE}" \
  --latent-cvae-canvas-cross-attention "${LATENT_CVAE_CANVAS_CROSS_ATTENTION}" \
  --adaptive-cvae-serial-writers "${ADAPTIVE_CVAE_SERIAL_WRITERS}" \
  --latent-cvae-trajectory-pinv "${LATENT_CVAE_TRAJECTORY_PINV}" \
  --latent-cvae-trajectory-ridge "${LATENT_CVAE_TRAJECTORY_RIDGE}" \
  --latent-cvae-trajectory-mid-refine-only "${LATENT_CVAE_TRAJECTORY_MID_REFINE_ONLY}" \
  --adaptive-cvae-direct-condition-residual "${ADAPTIVE_CVAE_DIRECT_CONDITION_RESIDUAL}" \
  --adaptive-cvae-condition-strength "${ADAPTIVE_CVAE_CONDITION_STRENGTH}" \
  --latent-cvae-noisy-ratio-max "${LATENT_CVAE_NOISY_RATIO_MAX}" \
  --latent-cvae-noisy-ratio-weight "${LATENT_CVAE_NOISY_RATIO_WEIGHT}" \
  --latent-cvae-trajectory-context-norm-max "${LATENT_CVAE_TRAJECTORY_CONTEXT_NORM_MAX}" \
  --latent-cvae-trajectory-pos-exempt "${LATENT_CVAE_TRAJECTORY_POS_EXEMPT}" \
  --block-action-denoise-matrix "${BLOCK_ACTION_DENOISE_MATRIX}" \
  "$@"
