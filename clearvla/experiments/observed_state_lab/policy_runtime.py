...
def flow_losses(system: V35PolicySystem, sample: dict[str, Tensor], output: dict[str, Tensor], trainer: V35PolicyTrainerConfig) -> dict[str, Tensor]:
    ...
    proposal = torch.nn.functional.smooth_l1_loss(output["proposal_action"], sample["policy_action"])
...
def evaluate_v35_policy(
    *,
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        noise = torch.randn(
            sample["policy_action"].shape, generator=generator, device=device, dtype=sample["visual"].dtype
        )
...
def train_v35_policy(
    *,
    ...
    for epoch in range(start_epoch, trainer.epochs + 1):
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            if trainer.log_every and batch_index % trainer.log_every == 0:
                print(
                    ...
                    f"grad={grad:.3e} lr={optimizer.param_groups[0]['lr']:.3e}", flush=True,
                )
        ...
        balanced = full + 0.25 * float(val_metrics["first8_rmse"]) + 0.10 * (1 - float(val_metrics.get("gripper_f1", 0.0)))
        save = []
        if full < best["full_mse"]:
            best["full_mse"] = full; save.append("best_full.pt")
        if balanced < best["balanced"]:
            best["balanced"] = balanced; save.append("best_balanced.pt")
        payload = {
            "schema": "clearvla-v35-policy-checkpoint-v1", "epoch": epoch, "global_step": global_step,
        ...
        (out_dir / "v35_policy_summary.json").write_text(
            json.dumps(jsonable({"schema": "clearvla-v35-policy-summary-v1", "best": best, "latest": record}), indent=2),
            encoding="utf-8",
        )
...
__all__ = [
    "V35PolicyTrainerConfig", "prepare_policy_sample", "evaluate_v35_policy", "train_v35_policy",
]
