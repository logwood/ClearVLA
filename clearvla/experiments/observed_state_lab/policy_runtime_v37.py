...
def train_v37_policy(
    *,
    ...
    for epoch in range(start_epoch, trainer.epochs + 1):
        system.train(); rows = []
        include_future = float(trainer.future_latent_loss_weight) > 0 and epoch >= int(trainer.future_latent_loss_start_epoch)
        ...
        val_metrics = evaluate_v37_policy(system=system, loader=val_loader, conditioner=conditioner, device=device, dtype=dtype, camera_names=camera_names, action_normalizer=action_normalizer, trainer=trainer, max_batches=trainer.max_val_batches)
        ...
        payload = {
            "schema": "clearvla-v37-policy-checkpoint-v1", "epoch": epoch, "global_step": global_step,
        ...
        (out_dir / "v37_policy_summary.json").write_text(json.dumps(jsonable({"schema": "clearvla-v37-policy-summary-v1", "best": best, "latest": record}), indent=2), encoding="utf-8")
