...
from clearvla.policy.codec import TransitionAwarePhysicalVelocityHead
...
class TransitionLatentFusion(nn.Module):
    ...
    def __init__(self, config: V363PolicyConfig) -> None:
        ...
        self.event_cross = nn.MultiheadAttention(
    ...
    def forward(self, action_tokens: Tensor, event_tokens: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if self.training and self.config.transition_event_dropout > 0:
            ...
            keep = (
                torch.rand(action_tokens.shape[0], 1, 1, device=action_tokens.device)
                >= self.config.transition_event_dropout
            )
...
class V363PolicySystem(nn.Module):
    def __init__(
        ...
        self.planner = PolicyLatentDiTPlannerV363(
            policy_config,
            world_hidden=world_config.hidden_size,
            world_tokens=world_config.world_tokens,
        )
        self.decoder = PlannerConditionedPhysicalActionExpertV363(
            policy_config,
            world_hidden=world_config.hidden_size,
            world_tokens=world_config.world_tokens,
        )
    ...
    @staticmethod
    def ablate_world_tokens(
        world: Tensor, mode: str = "normal", *, seed: int | None = None
    ) -> Tensor:
        ...
        if normalized == "noise":
            ...
            return (
                torch.randn(
                    world.shape, device=world.device, dtype=world.dtype, generator=generator
                )
                * scale
                + mean
            )
