...
def arm_motion_labels(
    system: V362PolicySystem, target_action: Tensor, action_state: Tensor, threshold: float
) -> Tensor:
...
def flow_losses(
    ...
    physical_error = (
        (output["pred_physical_velocity"] - output["target_physical_velocity"])
        .square()
        .mean(dim=-1)
    )
    ...
    *,
    ...
    for batch_index, batch in enumerate(loader, start=1):
        if max_batches and batch_index > max_batches:
            break
        sample = prepare_v362_policy_sample(
            batch,
            conditioner=conditioner,
            system=system,
            camera_names=camera_names,
...
def train_v362_policy(
    *,
    ...
    for epoch in range(start_epoch, trainer.epochs + 1):
        ...
        val_metrics = evaluate_v362_policy(
            system=system,
            loader=val_loader,
            conditioner=conditioner,
            device=device,
        ...
        payload = {
            "schema": "clearvla-v36-2-policy-checkpoint-v1",
            "epoch": epoch,
            "global_step": global_step,
        ...
        (out_dir / "v36_2_policy_summary.json").write_text(
            json.dumps(
                jsonable(
                    {"schema": "clearvla-v36-2-policy-summary-v1", "best": best, "latest": record}
                ),
                indent=2,
            ),
            encoding="utf-8",
