...
class V39PolicyConfig(V38PolicyConfig):
    ...
    hierarchical_mmdit_architecture_version: str = "serial_owned_v2"
    ...
    # 0 = no noisy value gate; 1 = a post-normalization value-domain schedule.
    ...
    def validate(self) -> None:
        ...
        if str(self.hierarchical_mmdit_architecture_version) != "serial_owned_v2":
            raise ValueError(
...
class OwnedHierarchicalActionBlock(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        self_update = gates[:, 0, None, None] * self.drop(self.self_out(self._merge_heads(self_attended)))
        ...
        action, noisy_update, noisy_weight = self._cross_update(
            action,
            noisy_tokens,
        ...
        action, stage_update, stage_weight = self._cross_update(
            action,
            stage_tokens,
        ...
        action, low_update, low_weight = self._cross_update(
            action,
            low_tokens,
            kv_proj=self.low_kv,
        ...
        action = self.out_norm(action + ffn_update)

        branch_rows = torch.stack([
            self._row_norm(self_update),
            self._row_norm(noisy_update),
            self._row_norm(stage_update),
            self._row_norm(low_update),
            self._row_norm(ffn_update),
        ], dim=-1)
        ...
        serial_cancellation = (
            1.0 - total_rows / branch_rows.sum(dim=-1).clamp_min(1e-8)
        ).clamp(0.0, 1.0).mean()
...
class HierarchicalMMDiTActionDecoder(nn.Module):
    ...
    def _gate_noisy_tokens(self, noisy: Tensor, time: Tensor) -> tuple[Tensor, Tensor | None, Tensor]:
        ...
        it POST-norm on the value stream where it cannot be cancelled.
        """
