...
def rollout_diagnostics(output: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    if "rollout_delta_pred_hold_action" in output:
        ...
        rows["rollout_effect_change_hold"] = (
            (
        ...
        rows["rollout_effect_change_shuffle"] = (
            (
                output["rollout_delta_pred"].float()
        ...
        rows["rollout_full_effect_change_hold"] = (
            (
        ...
        rows["rollout_full_effect_change_shuffle"] = (
            (
                output["rollout_effect_pred"].float()
...
def train_v38_policy(
    *,
    ...
    for epoch in range(start_epoch, trainer.epochs + 1):
        ...
        payload = {
            "schema": "clearvla-v38-policy-checkpoint-v1",
            "epoch": epoch,
            "global_step": global_step,
        ...
        (out_dir / "v38_policy_summary.json").write_text(
            json.dumps(
                jsonable(
                    {"schema": "clearvla-v38-policy-summary-v1", "best": best, "latest": record}
                ),
                indent=2,
            ),
            encoding="utf-8",
