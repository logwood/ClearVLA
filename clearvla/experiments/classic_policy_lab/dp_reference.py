...
class ConditionalResidualBlock1D(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, cond_dim: int, *, kernel_size: int = 3, n_groups: int = 8, cond_predict_scale: bool = False) -> None:
        super().__init__()
...
class ConditionalUnet1D(nn.Module):
    ...
    def __init__(self, *, input_dim: int, global_cond_dim: int | None, diffusion_step_embed_dim: int = 256, down_dims: tuple[int, ...] = (256, 512, 1024), kernel_size: int = 3, n_groups: int = 8, cond_predict_scale: bool = False) -> None:
        super().__init__()
        ...
        for index, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.down_modules.append(nn.ModuleList([
                ConditionalResidualBlock1D(dim_in, dim_out, cond_dim, kernel_size=kernel_size, n_groups=n_groups, cond_predict_scale=cond_predict_scale),
                ConditionalResidualBlock1D(dim_out, dim_out, cond_dim, kernel_size=kernel_size, n_groups=n_groups, cond_predict_scale=cond_predict_scale),
                nn.Identity() if is_last else Downsample1d(dim_out),
            ]))
        mid_dim = all_dims[-1]
        self.mid_modules = nn.ModuleList([
            ConditionalResidualBlock1D(mid_dim, mid_dim, cond_dim, kernel_size=kernel_size, n_groups=n_groups, cond_predict_scale=cond_predict_scale),
            ConditionalResidualBlock1D(mid_dim, mid_dim, cond_dim, kernel_size=kernel_size, n_groups=n_groups, cond_predict_scale=cond_predict_scale),
        ])
        ...
        for dim_in, dim_out in reversed_pairs:
            ...
            self.up_modules.append(nn.ModuleList([
                ConditionalResidualBlock1D(dim_out * 2, dim_in, cond_dim, kernel_size=kernel_size, n_groups=n_groups, cond_predict_scale=cond_predict_scale),
                ConditionalResidualBlock1D(dim_in, dim_in, cond_dim, kernel_size=kernel_size, n_groups=n_groups, cond_predict_scale=cond_predict_scale),
                Upsample1d(dim_in),
            ]))
...
class DDPMScheduler:
    ...
    def __init__(self, *, num_train_timesteps: int = 100, clip_sample: bool = True, prediction_type: str = "epsilon") -> None:
        if prediction_type not in {"epsilon", "sample"}:
            raise ValueError(prediction_type)
        self.config = SimpleNamespace(num_train_timesteps=int(num_train_timesteps), clip_sample=bool(clip_sample), prediction_type=prediction_type)
    ...
    def add_noise(self, original: Tensor, noise: Tensor, timesteps: Tensor) -> Tensor:
        alpha = self.alphas_cumprod.to(original.device)[timesteps].reshape(-1, *([1] * (original.ndim - 1)))
    ...
    def set_timesteps(self, num_inference_steps: int) -> None:
        ...
        values = torch.linspace(self.config.num_train_timesteps - 1, 0, num_inference_steps).round().long()
        ...
        self._previous = {value: (unique[index + 1] if index + 1 < len(unique) else -1) for index, value in enumerate(unique)}

    def step(self, model_output: Tensor, timestep: Tensor | int, sample: Tensor, *, generator: torch.Generator | None = None, deterministic: bool = True) -> Tensor:
        ...
        alpha_prod_prev = torch.tensor(1.0, device=device) if previous < 0 else self.alphas_cumprod.to(device)[previous]
        ...
        if not deterministic and previous >= 0:
            noise = torch.randn(sample.shape, dtype=sample.dtype, device=device, generator=generator)
...
class RandomOrCenterCrop(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            return torch.stack([frame[..., int(yi):int(yi) + crop_h, int(zi):int(zi) + crop_w] for frame, yi, zi in zip(x, y, z)], dim=0)
        y0 = (height - crop_h) // 2
        x0 = (width - crop_w) // 2
...
class DPImageObsEncoder(nn.Module):
    ...
    def __init__(self, *, camera_names: tuple[str, ...], state_dim: int, crop_hw: tuple[int, int] | None = (84, 84), group_norm: bool = True, resnet18_weights: Path | None = None, share_rgb_model: bool = False) -> None:
        super().__init__()
        ...
        if share_rgb_model:
            self.shared_encoder = ResNet18GlobalPool(group_norm=group_norm, weights=resnet18_weights)
            self.camera_encoders = nn.ModuleDict()
        else:
            self.shared_encoder = None
            self.camera_encoders = nn.ModuleDict({name: ResNet18GlobalPool(group_norm=group_norm, weights=resnet18_weights) for name in camera_names})
    ...
    def forward(self, images: Tensor, state: Tensor) -> Tensor:
        ...
        for index, name in enumerate(self.camera_names):
            ...
            encoder = self.shared_encoder if self.shared_encoder is not None else self.camera_encoders[name]
...
class DPReference(nn.Module):
    ...
    def __init__(self, config: DPReferenceConfig) -> None:
        ...
        self.scheduler = DDPMScheduler(num_train_timesteps=config.diffusion_train_steps, clip_sample=True, prediction_type="epsilon")
    ...
    def encode_observation(self, obs_image: Tensor, obs_state: Tensor) -> Tensor:
        ...
        features = self.obs_encoder(obs_image.reshape(batch * obs_steps, *obs_image.shape[2:]), obs_state.reshape(batch * obs_steps, -1))
    ...
    def compute_loss(self, obs_image: Tensor, obs_state: Tensor, actions: Tensor) -> Tensor:
        ...
        timesteps = torch.randint(0, self.config.diffusion_train_steps, (actions.shape[0],), device=actions.device)
        noisy = self.scheduler.add_noise(actions, noise, timesteps)
        pred = self.model(noisy, timesteps, global_cond=global_cond)
        return F.mse_loss(pred, noise)

    @torch.no_grad()
    def predict_trajectory(self, obs_image: Tensor, obs_state: Tensor, *, inference_steps: int, deterministic: bool = True, generator: torch.Generator | None = None) -> Tensor:
        global_cond = self.encode_observation(obs_image, obs_state)
        trajectory = torch.randn((obs_image.shape[0], self.config.prediction_horizon, self.config.action_dim), device=obs_image.device, dtype=obs_image.dtype, generator=generator)
        ...
        for timestep in self.scheduler.timesteps.to(obs_image.device):
            ...
            trajectory = self.scheduler.step(pred, timestep, trajectory, generator=generator, deterministic=deterministic)
        return trajectory

    @torch.no_grad()
    def predict_action(self, obs_image: Tensor, obs_state: Tensor, *, inference_steps: int, deterministic: bool = True) -> Tensor:
        trajectory = self.predict_trajectory(obs_image, obs_state, inference_steps=inference_steps, deterministic=deterministic)
