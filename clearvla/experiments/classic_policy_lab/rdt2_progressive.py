...
class PooledVisualFirstCorrector(nn.Module):
    ...
    def __init__(self, hidden_size: int, action_dim: int) -> None:
        super().__init__()
        self.delta = nn.Sequential(nn.Linear(3 * hidden_size, hidden_size), nn.SiLU(), nn.Linear(hidden_size, action_dim))
        self.gate = nn.Sequential(nn.Linear(3 * hidden_size, hidden_size), nn.SiLU(), nn.Linear(hidden_size, action_dim))
        nn.init.zeros_(self.delta[-1].weight)
        nn.init.zeros_(self.delta[-1].bias)
        nn.init.zeros_(self.gate[-1].weight)
        nn.init.zeros_(self.gate[-1].bias)

    def forward(self, *, state: Tensor, history: Tensor, dense_condition: Tensor | None) -> tuple[Tensor, Tensor]:
...
class ProgressiveRDTCore(nn.Module):
    def __init__(self, config: ProgressiveRDT2FMConfig, *, dtype: torch.dtype) -> None:
        ...
        self.blocks = nn.ModuleList([ProgressiveRDTBlock(idx, config) for idx in range(config.depth)])
        self.modulation = StageModulationBank(config)
        self.final_layer = FinalLayer(config.action_dim, {
            "hidden_size": config.hidden_size,
            "norm_eps": config.norm_eps,
        })
        ...
        self.visual_first_corrector = PooledVisualFirstCorrector(config.hidden_size, config.action_dim)
    ...
    def _initialize(self, dtype: torch.dtype) -> None:
        ...
        nn.init.zeros_(self.first_exit_head.fc2.weight); nn.init.zeros_(self.first_exit_head.fc2.bias)
        nn.init.zeros_(self.prefix_exit_head.fc2.weight); nn.init.zeros_(self.prefix_exit_head.fc2.bias)
        nn.init.zeros_(self.visual_first_corrector.delta[-1].weight); nn.init.zeros_(self.visual_first_corrector.delta[-1].bias)
        nn.init.zeros_(self.visual_first_corrector.gate[-1].weight); nn.init.zeros_(self.visual_first_corrector.gate[-1].bias)
        self.to(dtype=dtype)
        ...
        for layer_idx, block in enumerate(self.blocks):
            ...
            if completed == cfg.prefix_exit_layer:
                ...
                if visual_correction is None:
                    visual_correction, visual_gate = self.visual_first_corrector(state=state, history=history_condition, dense_condition=dense_condition)
                prefix = prefix.clone()
...
class ProgressiveRDT2FM(nn.Module):
    ...
    def __init__(self, config: ProgressiveRDT2FMConfig = ProgressiveRDT2FMConfig(), *, dtype: torch.dtype = torch.float32) -> None:
        super().__init__()
        config.validate()
        self.config = config
    ...
    def compute_loss(
        ...
        *,
        ...
        prefix_flow = _weighted_mse(output.prefix, bridge.target_velocity[:, : self.config.prefix_length], weights[: self.config.prefix_length])
        total = (
