...
def _optimizer(model: ResidualFlowLabModel, config: ResidualFlowTrainerConfig, phase: str) -> torch.optim.Optimizer:
...
def _assert_finite_output(output: ResidualFlowLabOutput, *, prefix: str) -> None:
    for name in ("residual_velocity", "endpoint_residual", "endpoint_actions", "learned_source", "trajectory_tokens"):
    ...
    *,
    ...
    for phase, epochs in trainer.phase_epochs.ordered():
        ...
        for phase_epoch in range(1, epochs + 1):
            ...
            for batch_index, raw in enumerate(train_loader, start=1):
                ...
                try:
                    ...
                    if phase == "source_pretrain":
                        ...
                        source, _ = model.predict_source(batch["past"], batch["prior"])
                        loss = source_pretrain_loss(source, batch["future"], huber_beta=loss_config.huber_beta)
                        ...
                        correct = model.predict_residual_velocity_prepared(
                            ...
                            residual_state=bridge_batch.residual_state, bridge_time=bridge_batch.time,
                            step_size=bridge_batch.step_size_hint, noise_level=bridge_batch.noise_level,
                        ...
                        result = residual_flow_loss(
                            correct=correct, wrong=wrong, target_actions=batch["future"],
                            target_velocity=bridge_batch.target_velocity, config=loss_config,
                        )
                        loss = result.total
                    ...
                    if trainer.grad_clip > 0:
                        returned = torch.nn.utils.clip_grad_norm_(parameters, trainer.grad_clip, error_if_nonfinite=True)
                    ...
                    save_nan_debug_bundle(
                        debug / "nan_debug_batch.pt", batch=batch, epoch=global_epoch,
                        batch_index=batch_index, global_step=global_step, phase=f"{phase}:{marker}", error=exc,
                    )
                    _save_checkpoint(
            ...
            if phase == "source_pretrain":
                val = _evaluate_source(model, val_loaders_by_mode["correct"], device=device, normalizer=normalizer)
                ...
                val_modes = {
                    mode: evaluate_residual_flow_model(model, loader, device=device, normalizer=normalizer, integration_steps=trainer.integration_steps)
                ...
                if correct_mse < best_mse:
                    ...
                    _save_checkpoint(
                        ...
                        global_epoch=global_epoch, global_step=global_step, history=history + [record], context=context,
                    )
            history.append(record)
            _save_checkpoint(
                checkpoints / f"checkpoint_{global_epoch:04d}_{phase}.pt",
                model=model, optimizer=optimizer, phase=phase, phase_epoch=phase_epoch,
    ...
    (out_dir / "residual_flow_lab_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    export_state = {key: value.detach().cpu() for key, value in model.state_dict().items()}
    if best_path is not None:
        export_state = torch.load(best_path, map_location="cpu", weights_only=False)["model_state_dict"]
    _atomic_torch_save(
        {
