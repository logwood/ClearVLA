...
class RDTLiteTrainerConfig:
    ...
    def validate(self) -> None:
        ...
        if min(self.log_every, self.rolling_window, self.sampling_steps, self.parameter_finite_check_every, self.optimizer_state_finite_check_every) <= 0:
...
def _scheduler_lambda(step: int, *, total_steps: int, warmup_steps: int, min_lr_ratio: float) -> float:
...
def train_rdt_lite_lab(
    *,
    ...
    trainer.validate(); loss_config.validate(); schedule_config.validate(); codecs.validate()
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lr_lambda=lambda step: _scheduler_lambda(step, total_steps=total_steps, warmup_steps=trainer.warmup_steps, min_lr_ratio=trainer.min_lr_ratio),
    )
        ...
        for batch_index, raw in enumerate(train_loader, start=1):
            ...
            try:
                ...
                if trainer.grad_clip > 0:
                    grad_raw = float(torch.nn.utils.clip_grad_norm_(parameters, trainer.grad_clip, error_if_nonfinite=True).detach().cpu())
                ...
                save_nan_debug_bundle(debug / "nan_debug_batch.pt", batch=batch, epoch=epoch, batch_index=batch_index, global_step=global_step, phase=f"rdt_lite:{marker}", error=exc)
                _save_checkpoint(checkpoints / "failure.pt", model=model, optimizer=optimizer, scheduler=scheduler, codecs=codecs, epoch=epoch, global_step=global_step, history=history, context=context, loss_config=loss_config, trainer=trainer, schedule_config=schedule_config)
                raise
            total = float(result.total.detach().cpu())
            rolling.append(total)
            values["loss"].append(total); values["grad_raw"].append(grad_raw); values["grad_clipped"].append(grad_clipped)
            values["clip_ratio"].append(grad_clipped / max(grad_raw, 1e-12)); values["lr"].append(float(optimizer.param_groups[0]["lr"]))
            for key, value in groups.items(): values[f"grad_{key}_raw"].append(value)
            for key, value in result.components.items(): values[f"component_{key}"].append(float(value.detach().cpu()))
            for key, value in result.diagnostics.items(): values[f"diag_{key}"].append(float(value.detach().cpu()))
            if batch_index % trainer.log_every == 0:
                primary = "clean_action" if loss_config.objective == "rdt_denoise" else "flow_velocity"
                print(
                    ...
                    f"grad={grad_raw:.3e}->{grad_clipped:.3e} clip={values['clip_ratio'][-1]:.3f} ", end="", flush=True,
                )
        print(flush=True)
        record: dict[str, Any] = {"epoch": epoch, "global_step": global_step, "seconds": time.perf_counter() - started}
        for key, items in values.items(): record[f"train_{key}"] = float(np.mean(items)) if items else 0.0
        val_modes = {
            mode: evaluate_rdt_lite_model(model, loader, objective=loss_config.objective, device=device, codecs=codecs, sampling_steps=trainer.sampling_steps, diffusion_schedule=diffusion_schedule)
            for mode, loader in val_loaders_by_mode.items()
        }
        dependency = visual_dependency_report(val_modes)
        record["val"] = val_modes; record["dependency"] = dependency
        correct = val_modes["correct"]
        mse = float(correct["full_mse"]); arm_first = float(correct.get("arm_first_rmse", float("inf")))
        print(
            ...
            f"cross_gap={dependency.get('cross_episode_gap', float('nan')):+.6f} event_cross_gap={dependency.get('cross_episode_event_gap', float('nan')):+.6f}", flush=True,
        )
        print(
            ...
            f"arm_first_deg_if_rad={float(correct.get('arm_first_rmse_deg_if_rad', float('nan'))):.3f}", flush=True,
        )
        history.append(record)
        for path in (checkpoints / f"checkpoint_{epoch:04d}.pt", checkpoints / "latest.pt"):
            _save_checkpoint(path, model=model, optimizer=optimizer, scheduler=scheduler, codecs=codecs, epoch=epoch, global_step=global_step, history=history, context=context, loss_config=loss_config, trainer=trainer, schedule_config=schedule_config)
        if mse < best_full_mse:
            best_full_mse = mse; best_paths["full"] = checkpoints / "best_full.pt"
            _save_checkpoint(best_paths["full"], model=model, optimizer=optimizer, scheduler=scheduler, codecs=codecs, epoch=epoch, global_step=global_step, history=history, context=context, loss_config=loss_config, trainer=trainer, schedule_config=schedule_config)
        if arm_first < best_arm_first:
            best_arm_first = arm_first; best_paths["arm_first"] = checkpoints / "best_arm_first.pt"
            _save_checkpoint(best_paths["arm_first"], model=model, optimizer=optimizer, scheduler=scheduler, codecs=codecs, epoch=epoch, global_step=global_step, history=history, context=context, loss_config=loss_config, trainer=trainer, schedule_config=schedule_config)
    ...
    summary = {
        ...
        "best_checkpoints": {key: None if path is None else str(path) for key, path in best_paths.items()},
        "parameter_count": model.parameter_count(), "history": history, "model": model.config.to_dict(),
        "trainer": asdict(trainer), "loss": asdict(loss_config), "diffusion_schedule": asdict(schedule_config), "codecs": codecs.to_dict(), "context": context,
    }
    (out_dir / "rdt_lite_lab_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    export_path = best_paths["full"] or checkpoints / "latest.pt"
    export_state = torch.load(export_path, map_location="cpu", weights_only=False)["model_state_dict"]
    _atomic_torch_save({
        "schema": "clearvla-rdt-lite-lab-export-v13.1", "model_state_dict": export_state,
        "model_config": model.config.to_dict(), "loss_config": asdict(loss_config), "trainer_config": asdict(trainer),
        "diffusion_schedule_config": asdict(schedule_config), "codecs": codecs.to_dict(), "summary": summary, "context": context,
