...
def main() -> None:
    ...
    episodes, train_ids, val_ids, test_ids, _, _, store, skipped = load_data(args, min_length=data_config.chunk_len + 1, normalizer_mode="zscore", action_normalizer=normalizer_action, state_normalizer=normalizer_state, splits=context["splits"])
    ids = {"train": train_ids, "val": val_ids, "test": test_ids}[args.split]
    ds = ACTWindowDataset(episodes, ids, image_store=store, camera_names=model_config.camera_names, state_normalizer=normalizer_state, action_normalizer=normalizer_action, config=data_config)
    loader = make_loader(ds, batch_size=args.batch_size, workers=args.num_workers, shuffle=False, device=device)
    model = ACTReference(model_config).to(device); model.load_state_dict(payload["model"])
    metrics = evaluate_act(model, loader, device=device, action_normalizer=normalizer_action, max_batches=args.max_val_batches)
    report = {"schema": "clearvla-act-reference-eval-v1", "checkpoint": str(args.checkpoint), "split": args.split, "metrics": metrics, "skipped": skipped}
