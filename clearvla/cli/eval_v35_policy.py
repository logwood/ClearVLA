...
from clearvla.experiments.observed_state_lab.policy_runtime import V35PolicyTrainerConfig, evaluate_v35_policy
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a V35 policy checkpoint.")
...
def main() -> None:
    ...
    policy_config = V35PolicyConfig(**payload["policy_config"])
    trainer = V35PolicyTrainerConfig(**payload["trainer_config"])
    ...
    system = V35PolicySystem(world_config, policy_config, WorldEvidenceEncoder(world_config))
    system.load_state_dict(payload["model"], strict=True)
    system.to(device=device, dtype=torch.float32)
    metrics = evaluate_v35_policy(
