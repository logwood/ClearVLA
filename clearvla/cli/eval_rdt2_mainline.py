...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate an RDT2 mainline checkpoint with optional contrastive future-DINO dynamics")
...
def main() -> None:
    ...
    data_config = dataclasses.replace(
        data_config,
        return_future_images=(model_config.future_latent_variant != "none" and mode != "dinov2-cache"),
    )
    ...
    episodes, train_ids, val_ids, test_ids, _, _, store, skipped = load_data(
        args, min_length=max(data_config.prediction_horizon, future_span) + 1, normalizer_mode=action_norm.mode,
    ...
    def run(mode_name: str):
        return evaluate_mainline_rdt2_fm(model, conditioner, loader, device=device, action_normalizer=action_norm, inference_steps=steps, max_batches=args.max_val_batches, instruction=instruction, image_ablation=mode_name)
    metrics = run(args.image_ablation)
    report = {"schema": "clearvla-rdt2-mainline-eval-v4-latent-dynamics", "checkpoint": str(args.checkpoint), "split": args.split, "condition_mode": mode, "image_ablation": args.image_ablation, "metrics": metrics, "skipped": skipped}
