...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dump V39 arm-motion and optional FK task-space tables for one episode."
    )
    ...
    parser.add_argument(
        "--urdf-variant", default="gripper_50mm", choices=["gripper_50mm", "gripper_100mm"]
    )
...
def main() -> None:
    ...
    if latent_dim != int(visual_geometry["latent_dim"]) or (
        patches is not None and patches != int(visual_geometry["patches_per_camera"])
    ):
