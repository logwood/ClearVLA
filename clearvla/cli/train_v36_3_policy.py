...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the V36.3 transition-aware action latent policy."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional V36.2/V36.3 policy checkpoint for model-only warm start.",
    )
    parser.add_argument(
        "--condition-mode",
...
def main() -> None:
    ...
    if args.init_policy_checkpoint is not None:
        init_payload = torch.load(
            args.init_policy_checkpoint, map_location="cpu", weights_only=False
        )
        if init_payload.get("schema") not in {
            "clearvla-v36-2-policy-checkpoint-v1",
            "clearvla-v36-3-policy-checkpoint-v1",
        }:
    ...
    trainer = V363PolicyTrainerConfig(
        **{name: getattr(args, name) for name in V363PolicyTrainerConfig.__dataclass_fields__}
    )
    context = {
        "schema": "clearvla-v36-3-transition-aware-action-latent-context-v1",
        "args": vars(args),
