...
from clearvla.experiments.observed_state_lab.policy import V35PolicyConfig, V35PolicySystem
from clearvla.experiments.observed_state_lab.policy_runtime import V35PolicyTrainerConfig, train_v35_policy
from clearvla.experiments.observed_state_lab.world_model import V35ObservedStateWorldModel, V35WorldConfig


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the V35 unified latent action expert.")
    ...
    parser.add_argument("--proposal-dropout", type=float, default=0.5)
    parser.add_argument("--inference-steps", type=int, default=5)
    parser.add_argument("--gripper-dim-index", type=int, default=-1)

    defaults = V35PolicyTrainerConfig()
    for field in V35PolicyTrainerConfig.__dataclass_fields__:
...
def main() -> None:
    ...
    policy_config = V35PolicyConfig(
        ...
        proposal_depth=args.proposal_depth, proposal_dropout=args.proposal_dropout,
        gripper_dim_index=args.gripper_dim_index, inference_steps=args.inference_steps,
    )
    system = V35PolicySystem(world_config, policy_config, deepcopy(world_model.online_encoder))
    trainer = V35PolicyTrainerConfig(**{name: getattr(args, name) for name in V35PolicyTrainerConfig.__dataclass_fields__})
    context = {
        "schema": "clearvla-v35-policy-context-v1", "args": vars(args),
        ...
        "policy_contract": "one full-depth unified 7-D flow expert; proposal is detached/rejectable and never an execution residual",
    }
