...
from clearvla.experiments.observed_state_lab.policy_runtime_v37 import V37PolicyTrainerConfig, train_v37_policy
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--world-checkpoint", type=Path, required=True, help="V35/V35.6 checkpoint used only for dataset/normalizer/geometry context.")
...
def main() -> None:
    ...
    if payload.get("schema") != "clearvla-v35-world-checkpoint-v1":
        raise ValueError("--world-checkpoint must be a V35 world checkpoint for dataset/normalizer context")
    ...
    policy_config = V37PolicyConfig(
        ...
        action_decoder_depth=1, proposal_depth=args.proposal_depth,
        ...
        visual_token_dim=world_config.latent_dim, visual_history_length=world_config.history_length,
        num_cameras=world_config.num_cameras, patches_per_camera=world_config.patches_per_camera,
        high_level_slots=args.high_level_slots, future_aux_offsets=min(int(args.future_aux_offsets), world_config.num_future),
    ...
    trainer = V37PolicyTrainerConfig(**{name: getattr(args, name) for name in V37PolicyTrainerConfig.__dataclass_fields__})
    context = {
        "schema": "clearvla-v37-full-latent-world-shaped-context-v1", "args": vars(args),
        "source_world_checkpoint": str(args.world_checkpoint), "source_world_checkpoint_epoch": payload["epoch"],
        "splits": world_context["splits"], "dataset": asdict(dataset_config), "source_world_model": asdict(world_config),
    ...
    train_v37_policy(
        ...
        device=device, dtype=dtype, camera_names=cameras, action_normalizer=action_norm, state_normalizer=state_norm,
        trainer=trainer, out_dir=args.out_dir, context=context, resume=args.resume,
    )
