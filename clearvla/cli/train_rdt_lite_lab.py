...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train corrected lightweight RDT-style direct action references")
    ...
    p.add_argument("--action-representation", choices=["absolute", "relative_to_current"], default="relative_to_current")
    ...
    p.add_argument("--sampler", choices=["shuffled-block", "sequential", "event-block"], default="shuffled-block")
...
def _loader(dataset: RDTLiteDataset, *, batch_size: int, workers: int, device: torch.device) -> DataLoader:
...
def main() -> None:
    ...
    min_length = max(args.past_len, args.state_history_len, args.obs_horizon) + args.chunk_len + max(abs(args.state_offset), abs(args.image_offset), abs(args.action_offset))
    ...
    codecs = fit_rdt_lite_codecs(
        ...
        chunk_len=args.chunk_len, past_len=args.past_len, state_history_len=args.state_history_len, obs_horizon=args.obs_horizon,
        state_offset=args.state_offset, image_offset=args.image_offset, action_offset=args.action_offset, stride=args.stride,
    )
    ...
    data_config = RDTLiteDatasetConfig(
        chunk_len=args.chunk_len, past_len=args.past_len, state_history_len=args.state_history_len, obs_horizon=args.obs_horizon,
        stride=args.stride, state_offset=args.state_offset, image_offset=args.image_offset, action_offset=args.action_offset,
        prior=args.prior, prior_beta=args.prior_beta, velocity_mode=args.velocity_mode, ema_decay=args.ema_decay, visual_shift=args.visual_shift,
    )
    train_ds = RDTLiteDataset(episodes, train_ids, latent_store=latent_store, codecs=codecs, config=data_config, visual_mode=LabVisualMode.CORRECT, visual_pool_episode_ids=train_ids)
    train_scores = compute_rdt_lite_event_scores(train_ds, LabEventScoreConfig(event_quantile=args.event_quantile)); train_ds.attach_event_scores(train_scores)
    if args.sampler == "shuffled-block":
        batch_sampler = TrajectoryShuffledBlockBatchSampler(train_ds.refs, TrajectoryShuffledBlockSamplerConfig(block_size=args.batch_size, seed=args.seed))
    elif args.sampler == "sequential":
        batch_sampler = TrajectorySequentialBatchSampler(train_ds.refs, TrajectorySequentialSamplerConfig(block_size=args.batch_size, seed=args.seed))
    else:
        batch_sampler = TrajectoryBlockBatchSampler(train_ds.refs, train_scores.is_event, TrajectoryBlockSamplerConfig(block_size=args.batch_size, event_fraction=args.event_fraction, seed=args.seed))
    train_loader = DataLoader(train_ds, batch_sampler=batch_sampler, num_workers=args.num_workers, pin_memory=device.type == "cuda")

    val_visual_pool = val_ids if len(val_ids) > 1 else list(range(len(episodes)))
    val_loaders: dict[str, DataLoader] = {}
    for mode in LabVisualMode:
        ds = RDTLiteDataset(episodes, val_ids, latent_store=latent_store, codecs=codecs, config=data_config, visual_mode=mode, visual_pool_episode_ids=val_visual_pool)
        ds.attach_event_scores(compute_rdt_lite_event_scores(ds, LabEventScoreConfig(event_quantile=args.event_quantile)))
        val_loaders[mode.value] = _loader(ds, batch_size=args.batch_size, workers=args.num_workers, device=device)

    time_encoding = "rdt_discrete" if args.objective == "rdt_denoise" else "pi_continuous"
    model_config = RDTLiteModelConfig(
        state_dim=int(episodes[0].states_raw.shape[1]), action_dim=int(episodes[0].actions_raw.shape[1]),
        chunk_len=args.chunk_len, obs_horizon=args.obs_horizon, state_history_len=args.state_history_len,
        camera_names=cameras, camera_order=camera_order, patch_grid=latent_meta.patch_grid, teacher_dim=latent_meta.token_dim,
        hidden_size=args.hidden_size, depth=args.depth, num_heads=args.num_heads, ffn_hidden=args.ffn_hidden,
        img_adaptor_depth=args.img_adaptor_depth, state_adaptor_depth=args.state_adaptor_depth, action_adaptor_depth=args.action_adaptor_depth,
        conditioning_mode=args.conditioning_mode, independent_camera_dropout=args.camera_dropout,
        include_visual_delta_tokens=args.include_visual_delta_tokens, dropout=args.dropout, time_encoding=time_encoding,
        control_frequency_hz=args.control_frequency_hz, decoder_output_init_std=args.decoder_output_init_std,
    )
    ...
    loss_config = RDTLiteLossConfig(objective=args.objective, first_weight=args.first_weight, first4_weight=args.first4_weight, gripper_weight=args.gripper_weight, pi_endpoint_weight=args.pi_endpoint_weight, pi_time_alpha=args.pi_time_alpha, pi_time_beta=args.pi_time_beta)
    trainer = RDTLiteTrainerConfig(epochs=args.epochs, lr=args.lr, weight_decay=args.weight_decay, grad_clip=args.grad_clip, warmup_steps=args.warmup_steps, min_lr_ratio=args.min_lr_ratio, log_every=args.log_every, rolling_window=args.rolling_window, sampling_steps=sampling_steps, max_train_batches=args.max_train_batches)
    context = {
    ...
    print(json.dumps({
        "data": {"episodes": len(episodes), "train_windows": len(train_ds), "train_batches": len(train_loader), "sampler": args.sampler},
        "alignment": {"state_offset": args.state_offset, "image_offset": args.image_offset, "action_offset": args.action_offset},
        "action_representation": codecs.action_representation,
        "model": {"parameters": model.parameter_count(), "conditioning": args.conditioning_mode, "time_encoding": time_encoding},
    }, indent=2))
    if args.dry_run:
        raw = next(iter(train_loader)); batch = {key: value.to(device) for key, value in raw.items()}
        result = compute_rdt_lite_loss(model, state_history=batch["state_history"], target_actions=batch["target_actions"], visual_tokens=batch["visual_tokens"], config=loss_config, diffusion_schedule=CosineDiffusionSchedule(schedule_config))
        result.total.backward()
        print("dry_run_shapes:", {key: tuple(value.shape) for key, value in batch.items() if hasattr(value, "shape")})
        print("dry_run_components:", {key: float(value.detach().cpu()) for key, value in result.components.items()})
        print("dry-run passed")
        return
    args.out_dir.mkdir(parents=True, exist_ok=True)
    (args.out_dir / "train_context.json").write_text(json.dumps(context, indent=2), encoding="utf-8")
    resume_payload = torch.load(args.resume, map_location="cpu", weights_only=False) if args.resume else None
    summary = train_rdt_lite_lab(model=model, train_loader=train_loader, val_loaders_by_mode=val_loaders, device=device, codecs=codecs, out_dir=args.out_dir, trainer=trainer, loss_config=loss_config, schedule_config=schedule_config, context=context, resume_payload=resume_payload)
    print(json.dumps({"best_full_mse": summary["best_full_mse"], "best_arm_first_rmse": summary["best_arm_first_rmse"], "summary": str(args.out_dir / "rdt_lite_lab_summary.json")}, indent=2))
