...
from clearvla.experiments.observed_state_lab.policy_runtime_v36_2 import V362PolicyTrainerConfig
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dump arm-motion and optional FK task-space tables for one V36.2 episode.")
    ...
    parser.add_argument("--phase-merge-gap", type=int, default=8, help="Merge micro motion peaks into a phase when the gap is at most this many frames.")
    parser.add_argument("--ratio-eps", type=float, default=1e-6, help="Denominator floor below which ratio/progress metrics are left blank.")
    parser.add_argument("--disable-fk", action="store_true", help="Only write joint-space metrics; skip URDF FK.")
    parser.add_argument("--urdf-path", type=Path, default=None, help="Explicit Alicia-D URDF. Overrides the bundled fallback.")
    parser.add_argument("--urdf-variant", default="gripper_50mm", choices=["gripper_50mm", "gripper_100mm"], help="Bundled v5.6 fallback variant when --urdf-path is omitted.")
