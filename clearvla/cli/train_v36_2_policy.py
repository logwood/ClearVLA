...
from clearvla.experiments.observed_state_lab.policy_runtime_v36_2 import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--action-decoder-depth",
        type=int,
        default=4,
        help="planner-conditioned physical action decoder depth",
    )
...
def main() -> None:
    ...
    trainer = V362PolicyTrainerConfig(
        **{name: getattr(args, name) for name in V362PolicyTrainerConfig.__dataclass_fields__}
    )
    context = {
        "schema": "clearvla-v36-2-physical-action-flow-context-v1",
        "args": vars(args),
        "world_checkpoint": str(args.world_checkpoint),
        "world_checkpoint_epoch": world_payload["epoch"],
        "splits": world_context["splits"],
        "dataset": asdict(dataset_config),
        "world_model": asdict(world_config),
