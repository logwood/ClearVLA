...
from clearvla.experiments.observed_state_lab.policy_runtime_v36_2 import V362PolicyTrainerConfig, train_v362_policy
from clearvla.experiments.observed_state_lab.world_model import V35ObservedStateWorldModel, V35WorldConfig


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the V36.2 physical-action-flow policy.")
    ...
    defaults = V362PolicyTrainerConfig()
    for field in V362PolicyTrainerConfig.__dataclass_fields__:
...
def main() -> None:
    ...
    system = V362PolicySystem(world_config, policy_config, deepcopy(world_model.online_encoder))
    trainer = V362PolicyTrainerConfig(**{name: getattr(args, name) for name in V362PolicyTrainerConfig.__dataclass_fields__})
    context = {
        "schema": "clearvla-v36-2-physical-action-flow-context-v1", "args": vars(args),
        ...
        "skipped": skipped,
        "policy_contract": (
            "frozen V35 world encoder; V36.2 typed physical action flow over arm_abs/arm_delta/gripper_value/gripper_delta; "
            "typed action lift and typed physical velocity head; explicit first/mid/tail horizon-role embeddings; "
            "native Alicia-D 7-D decoder; auxiliary gripper event and arm motion heads"
        ),
    }
    print_context(context)
    train_v362_policy(system=system, train_loader=train_loader, val_loader=val_loader, conditioner=conditioner,
