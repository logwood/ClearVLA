...
from clearvla.experiments.observed_state_lab.intervention import InterventionBranchDataset, validate_intervention_groups
from clearvla.experiments.observed_state_lab.world_model import V35ObservedStateWorldModel, V35WorldConfig
from clearvla.experiments.observed_state_lab.world_runtime import prepare_v35_sample, forward_v35, jsonable
...
def main() -> None:
    ...
    episodes, *_ = load_data(
        args, min_length=2, normalizer_mode=action_norm.mode,
        action_normalizer=action_norm, state_normalizer=state_norm, splits=context["splits"],
    )
    dataset = InterventionBranchDataset(
        args.intervention_npz, action_normalizer=action_norm, state_normalizer=state_norm,
    ...
    conditioner, latent_dim, patches = build_dense_conditioner(
        ...
        debug_token_dim=cfg.latent_dim, debug_patches_per_camera=cfg.patches_per_camera,
    ...
    for batch in loader:
        sample = prepare_v35_sample(
            batch, conditioner=conditioner, model=model, camera_names=cameras,
            device=device, dtype=dtype,
        )
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=dtype, enabled=device.type == "cuda" and dtype != torch.float32):
    ...
    for rows in groups.values():
        for left, right in combinations(rows, 2):
            pred_delta = (left["pred"] - left["initial"][None]) - (right["pred"] - right["initial"][None])
            target_delta = (left["target"] - left["initial"][None]) - (right["target"] - right["initial"][None])
            cosines.append(float(F.cosine_similarity(pred_delta.flatten(), target_delta.flatten(), dim=0)))
            magnitude_ratios.append(float(pred_delta.norm() / target_delta.norm().clamp_min(1e-8)))
        for row in rows:
            state_rmse.append(float((row["state_pred"] - row["state_target"]).square().mean().sqrt()))
    metrics = {
        "groups": len(groups), "branches": len(outputs), "branch_pairs": len(cosines),
