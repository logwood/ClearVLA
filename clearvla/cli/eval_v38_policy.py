...
from clearvla.experiments.observed_state_lab.policy_runtime_v38 import V38PolicyTrainerConfig, evaluate_v38_policy
from clearvla.experiments.observed_state_lab.world_runtime import jsonable


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate V38 temporal world-action DiT policy.")
...
def main() -> None:
    ...
    if payload.get("schema") != "clearvla-v38-policy-checkpoint-v1":
        raise ValueError("--checkpoint must be a V38 policy checkpoint")
    ...
    policy_config = V38PolicyConfig(**payload["policy_config"])
    trainer = V38PolicyTrainerConfig(**payload["trainer_config"])
    if args.eval_inference_steps is not None:
        trainer = V38PolicyTrainerConfig(**{**asdict(trainer), "eval_inference_steps": int(args.eval_inference_steps)})
    ...
    out = {"schema": "clearvla-v38-policy-eval-v1", "split": args.split, "checkpoint": str(args.checkpoint), "metrics": metrics, "skipped": skipped}
