...
def main() -> None:
    ...
    dataset = PairedDynamicWorldDataset(
        base,
        pair_index=table.pair_index,
        pair_valid=table.pair_valid,
        pair_distance=table.pair_distance,
        action_distance=table.action_distance,
    ...
    if latent_dim != model_config.latent_dim or (
        patches is not None and patches != model_config.patches_per_camera
    ):
    ...
    metrics = evaluate_v35_world(
        ...
        dtype=dtype,
        camera_names=cameras,
        loss_config=V35WorldLossConfig(**loss_payload),
        state_normalizer=state_norm,
        action_normalizer=action_norm,
