...
from clearvla.experiments.observed_state_lab.dataset import (
    CurrentEvidenceViewDataset, ObservedStateDatasetConfig, ObservedStateWindowDataset,
)
...
from clearvla.experiments.observed_state_lab.world_runtime import V35WorldTrainerConfig, train_v35_world
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train V35 observed-state segment-recurrent latent dynamics.")
    ...
    parser.add_argument("--rollout-checkpoint", action="store_true", help="Checkpoint each segment-recurrent rollout step during training to reduce activation memory.")
    ...
    parser.add_argument("--no-rollout-checkpoint-preserve-rng-state", dest="rollout_checkpoint_preserve_rng_state", action="store_false", help="Do not preserve RNG state inside rollout checkpoint recomputation. Leave default on unless the transition has no stochastic layers.")
    ...
    for name in (
        "epochs", "encoder_lr", "dynamics_lr", "auxiliary_lr", "weight_decay", "beta1", "beta2",
        "eps", "grad_clip", "warmup_steps", "action_warmup_steps", "stability_warmup_steps",
        "min_lr_ratio", "ema_decay_start", "ema_decay_end", "camera_drop_prob", "state_mask_prob",
        "executed_action_mask_prob", "patch_mask_prob", "checkpoint_predictive_slack",
        "checkpoint_hold_ratio_max", "checkpoint_min_embedding_std", "checkpoint_zero_world_max",
        "log_every", "max_train_batches", "max_val_batches", "eval_ablation_batches",
    ):
        value = getattr(trainer, name)
        parser.add_argument("--" + name.replace("_", "-"), dest=name, type=type(value), default=value)
    return parser.parse_args()


@torch.no_grad()
def pair_descriptors(
    dataset, *, conditioner, model, cameras, device, dtype, batch_size, workers,
    gripper_open_value: float, gripper_close_value: float,
):
    loader = DataLoader(
        CurrentEvidenceViewDataset(dataset), batch_size=batch_size, shuffle=False,
    ...
    for batch in loader:
        tokens = encode_current_tokens(
            batch, conditioner=conditioner, model_config=model.config, camera_names=cameras,
...
def build_or_load_pairs(pair_dir: Path, bases: dict[str, ObservedStateWindowDataset], **kwargs):
    ...
    expected = {
        ...
        "train_windows": len(bases["train"]), "val_windows": len(bases["val"]), "test_windows": len(bases["test"]),
    ...
    descriptor_kwargs = {
        key: kwargs[key]
        for key in ("conditioner", "model", "cameras", "device", "dtype", "batch_size", "workers", "gripper_open_value", "gripper_close_value")
    }
    ...
    for name, (condition, action, future, episode, gripper) in rows.items():
        tables[name] = build_local_pair_table(
            condition_descriptor=condition, action_summary=action, future_summary=future,
            episode_ids=episode, gripper_state=gripper,
...
def main() -> None:
    ...
    dataset_config = ObservedStateDatasetConfig(
        world_horizon=args.world_horizon, policy_horizon=args.policy_horizon,
        segment_length=args.segment_length, history_offsets=tuple(args.history_offsets),
        executed_action_offsets=tuple(args.executed_action_offsets),
        target_history_offsets=tuple(args.target_history_offsets), state_offset=args.state_offset,
        image_offset=args.image_offset, action_offset=args.action_offset, stride=args.stride,
        return_images=args.condition_mode != "dinov2-cache",
    )
    dataset_config.validate()
    min_length = args.world_horizon + abs(min(args.history_offsets + args.executed_action_offsets)) + 2
    episodes, train_ids, val_ids, test_ids, action_norm, state_norm, image_store, skipped = load_data(
        args, min_length=min_length, normalizer_mode=args.normalizer
    )
    bases = {
        name: ObservedStateWindowDataset(
            episodes, ids, image_store=image_store, camera_names=cameras,
            state_normalizer=state_norm, action_normalizer=action_norm, config=dataset_config,
        )
    ...
    conditioner, latent_dim, patches = build_dense_conditioner(
        ...
        debug_token_dim=args.debug_token_dim, debug_patches_per_camera=args.debug_patches_per_camera,
        device=device, dtype=dtype,
    )
    if patches is None:
        latent_dim, patches = infer_dense_geometry(conditioner, bases["train"][0], camera_names=cameras)
    model_config = V35WorldConfig(
        latent_dim=latent_dim, action_dim=int(action_norm.scale.shape[-1]), state_dim=int(state_norm.scale.shape[-1]),
        world_horizon=args.world_horizon, segment_length=args.segment_length,
        history_length=len(args.history_offsets), executed_history_length=len(args.executed_action_offsets),
        num_cameras=len(cameras), patches_per_camera=patches, hidden_size=args.hidden_size,
        num_heads=args.heads, world_tokens=args.world_tokens, global_tokens=args.global_tokens,
        interaction_tokens=args.interaction_tokens, motion_tokens=args.motion_tokens,
        evidence_read_depth=args.evidence_read_depth, latent_mix_depth=args.latent_mix_depth,
        action_depth=args.action_depth, transition_depth=args.transition_depth,
        transition_unique_blocks=args.transition_unique_blocks,
        inverse_depth=args.inverse_depth, descriptor_projection_dim=args.descriptor_projection_dim,
    ...
    tables = build_or_load_pairs(
        pair_dir, bases, conditioner=conditioner, model=model, cameras=cameras, device=device, dtype=dtype,
        batch_size=args.pair_build_batch_size, workers=args.num_workers,
        gripper_open_value=args.gripper_open_value, gripper_close_value=args.gripper_close_value,
        candidate_count=args.pair_candidates, min_action_distance=args.pair_min_action_distance,
        min_future_distance=args.pair_min_future_distance, rebuild=args.rebuild_pairs,
    )
    datasets = {
        name: PairedDynamicWorldDataset(
            base, pair_index=tables[name].pair_index, pair_valid=tables[name].pair_valid,
            pair_distance=tables[name].pair_distance, action_distance=tables[name].action_distance,
            future_distance=tables[name].future_distance,
        )
        for name, base in bases.items()
    }
    train_loader = make_loader(datasets["train"], batch_size=args.batch_size, workers=args.num_workers, shuffle=True, device=device)
    val_loader = make_loader(datasets["val"], batch_size=args.batch_size, workers=args.num_workers, shuffle=False, device=device)
    loss_config = V35WorldLossConfig(**{name: getattr(args, name) for name in V35WorldLossConfig.__dataclass_fields__})
    trainer = V35WorldTrainerConfig(**{name: getattr(args, name) for name in V35WorldTrainerConfig.__dataclass_fields__})
    context = {
        "schema": "clearvla-v35-observed-state-world-context-v1",
        "args": vars(args), "splits": {"train": train_ids, "val": val_ids, "test": test_ids},
        "skipped": skipped, "dataset": asdict(dataset_config), "model": asdict(model_config),
        "loss": asdict(loss_config), "trainer": asdict(trainer),
        "train_windows": len(bases["train"]), "val_windows": len(bases["val"]), "test_windows": len(bases["test"]),
        "pair_index_dir": pair_dir, "parameter_report": model.parameter_report(),
    ...
    train_v35_world(
        model=model, train_loader=train_loader, val_loader=val_loader, conditioner=conditioner,
        device=device, dtype=dtype, camera_names=cameras, out_dir=args.out_dir, trainer=trainer,
        loss_config=loss_config, action_normalizer=action_norm, state_normalizer=state_norm,
        context=context, resume=args.resume,
    )
