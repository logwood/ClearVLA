...
from clearvla.experiments.observed_state_lab.policy_runtime_v36 import V36PolicyTrainerConfig, train_v36_policy
from clearvla.experiments.observed_state_lab.world_model import V35ObservedStateWorldModel, V35WorldConfig


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the V36 policy-side latent DiT action planner.")
    ...
    parser.add_argument("--action-decoder-depth", type=int, default=4, help="V35-style action expert decoder depth")
    ...
    defaults = V36PolicyTrainerConfig()
    for field in V36PolicyTrainerConfig.__dataclass_fields__:
...
def main() -> None:
    ...
    train_loader = make_loader(
        PolicyWindowDataset(bases["train"]), batch_size=args.batch_size, workers=args.num_workers,
        shuffle=True, device=device,
    )
    val_loader = make_loader(
        PolicyWindowDataset(bases["val"]), batch_size=args.batch_size, workers=args.num_workers,
    ...
    policy_config = V36PolicyConfig(
        ...
        action_decoder_depth=args.action_decoder_depth, proposal_depth=args.proposal_depth, proposal_dropout=args.proposal_dropout,
        dropout=args.dropout, event_tokens=args.event_tokens,
        gripper_dim_index=args.gripper_dim_index, inference_steps=args.inference_steps,
    )
    system = V36PolicySystem(world_config, policy_config, deepcopy(world_model.online_encoder))
    trainer = V36PolicyTrainerConfig(**{name: getattr(args, name) for name in V36PolicyTrainerConfig.__dataclass_fields__})
    context = {
        "schema": "clearvla-v36-policy-latent-dit-context-v1", "args": vars(args),
        "world_checkpoint": str(args.world_checkpoint), "world_checkpoint_epoch": world_payload["epoch"],
        "splits": world_context["splits"], "dataset": asdict(dataset_config),
        "world_model": asdict(world_config), "policy_model": asdict(policy_config),
        "trainer": asdict(trainer), "parameter_report": system.parameter_report(), "skipped": skipped,
        "policy_contract": (
            "frozen V35 world encoder; typed latent DiT planner; V35-style planner-conditioned "
            "action expert decoder; unified 7-D flow head; auxiliary hold/open/close gripper "
            "event head; balanced/deploy checkpoint selectors"
        ),
    }
    print_context(context)
    train_v36_policy(
        system=system, train_loader=train_loader, val_loader=val_loader, conditioner=conditioner,
