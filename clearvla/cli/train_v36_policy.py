...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the V36 policy-side latent DiT action planner."
    )
    ...
    parser.add_argument(
        "--action-decoder-depth", type=int, default=4, help="V35-style action expert decoder depth"
    )
...
def main() -> None:
    ...
    trainer = V36PolicyTrainerConfig(
        **{name: getattr(args, name) for name in V36PolicyTrainerConfig.__dataclass_fields__}
    )
    context = {
        "schema": "clearvla-v36-policy-latent-dit-context-v1",
        "args": vars(args),
