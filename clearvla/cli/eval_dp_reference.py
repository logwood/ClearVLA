...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--use-raw-model", action="store_true", help="Evaluate raw weights instead of EMA weights when present")
    p.add_argument("--stochastic-sampling", action="store_true", help="Use DDPM posterior noise instead of deterministic posterior means")
    return p.parse_args()
...
def _model_config(data: dict) -> DPReferenceConfig:
    ...
    values["resnet18_weights"] = None if values.get("resnet18_weights") is None else Path(values["resnet18_weights"])
...
def main() -> None:
    ...
    episodes, train_ids, val_ids, test_ids, _, _, store, skipped = load_data(args, min_length=data_config.prediction_horizon + data_config.obs_horizon, normalizer_mode="limits", action_normalizer=normalizer_action, state_normalizer=normalizer_state, splits=context["splits"])
    ids = {"train": train_ids, "val": val_ids, "test": test_ids}[args.split]
    ds = DPWindowDataset(episodes, ids, image_store=store, camera_names=model_config.camera_names, state_normalizer=normalizer_state, action_normalizer=normalizer_action, config=data_config)
    ...
    weights = payload["model"] if args.use_raw_model or payload.get("ema_model") is None else payload["ema_model"]
    model.load_state_dict(weights)
    metrics = evaluate_dp(model, loader, device=device, action_normalizer=normalizer_action, inference_steps=args.inference_steps, max_batches=args.max_val_batches, deterministic=not args.stochastic_sampling)
    report = {"schema": "clearvla-dp-reference-eval-v1", "checkpoint": str(args.checkpoint), "split": args.split, "metrics": metrics, "skipped": skipped}
