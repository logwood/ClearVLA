...
class DecodedImageStore:
    ...
    def validate_episode(self, episode: LoadedEpisode) -> DecodedImageEpisodeMeta:
        ...
        if meta.num_frames != episode.cached_frame_count:
            raise ValueError(
                "decoded-image frame count mismatch: "
                f"{meta.num_frames} != {episode.cached_frame_count} cached rows"
            )
