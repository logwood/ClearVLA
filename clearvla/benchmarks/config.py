...
def build_benchmark_config(
    ...
    *,
    ...
    libero_retarget_overlay_root: Path | None = None,
    libero_retarget_overlay_manifest: Path | None = None,
) -> dict[str, Any]:
    ...
    if libero_retarget_overlay_manifest is not None and libero_retarget_overlay_root is None:
        raise ValueError("a LIBERO retarget overlay manifest requires its overlay root")
    if libero_retarget_overlay_root is not None:
        if (
            dataset_manifest is None
            or str(dataset_manifest.get("benchmark", "")).upper() != "LIBERO"
        ):
            raise ValueError("a LIBERO retarget overlay requires a converted LIBERO root")
        overlay_root = libero_retarget_overlay_root.resolve()
        overlay_manifest = (
            (overlay_root / "overlay_manifest.json").resolve()
            if libero_retarget_overlay_manifest is None
            else libero_retarget_overlay_manifest.resolve()
        )
        if not overlay_root.is_dir():
            raise FileNotFoundError(f"LIBERO retarget overlay root is absent: {overlay_root}")
        if not overlay_manifest.is_file():
            raise FileNotFoundError(
                f"LIBERO retarget overlay manifest is absent: {overlay_manifest}"
            )
        data["libero_retarget_overlay_root"] = str(overlay_root)
        data["libero_retarget_overlay_manifest"] = str(overlay_manifest)
...
def main() -> None:
    ...
    parser.add_argument("--libero-retarget-overlay-root", type=Path, default=None)
    parser.add_argument("--libero-retarget-overlay-manifest", type=Path, default=None)
    ...
    payload = build_benchmark_config(
        ...
        libero_retarget_overlay_root=args.libero_retarget_overlay_root,
        libero_retarget_overlay_manifest=args.libero_retarget_overlay_manifest,
    )
