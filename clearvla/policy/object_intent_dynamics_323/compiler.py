"""P2 consequence read and P3 five-lane policy compilation."""
...
class ObjectPolicyPlanDeltaBank:
    """Five explicit lanes; the factual consequence remains protected."""
...
class ObjectFutureEffectReader(nn.Module):
    """Bounded P2 read over interval-by-object effects with a null value."""
    ...
    def forward(
        ...
        *,
        ...
        effect_key = self._bounded_unit(
            self.effect_key(dynamics.semantic_delta)
        )
        ...
        # Validity is a selector mask only.  Reliability/uncertainty never
        # attenuate the value a second or third time.
        validity = dynamics.validity.float().squeeze(-1)
        logit = bounded_logit + validity.clamp_min(1e-6).log()[:, None, None]
        flat_logit = logit.flatten(-2)
        null_logit = torch.zeros_like(flat_logit[..., :1])
        posterior = torch.softmax(torch.cat((flat_logit, null_logit), dim=-1), dim=-1)
        # Preserve type ownership until the action query has selected an
        # interval/object source.  Absolute covariance and uncertainty remain
        # calibration fields; unlike these zero-centred changes they must not
        # become nonzero default policy values.
        ...
        type_weight = torch.softmax(
            self.type_query(action_query).float(), dim=-1
        )
        value = torch.einsum(
            "btqs,btqsh->btqh",
        ...
        metrics: dict[str, Tensor] = {
            ...
            # This excludes the validity log-mask, whose large negative value
            # is a selector exclusion rather than an unbounded learned score.
...
class ZeroPreservingObjectConsequence(nn.Module):
    ...
    def forward(
        self, *, factual_base: Tensor, effect: Tensor
...
class ObjectPolicyPlanCompiler(nn.Module):
    """P3 compiles factual/precision/effect/temporal/state-change lanes."""
    ...
    def forward(
        ...
        *,
        ...
        # The fifth lane is a weak, time/basis-specific modulation.  Its
