...
class MainlineTrainingEngine:
    ...
    def train_step(
        ...
        *,
        ...
        # A non-finite scalar can have finite derivatives (for example a
        # detached target/constant overflow). Gradient checks alone therefore
        # do not protect the optimizer from committing an invalid loss step.
        # Reject it, without substituting values or changing the objective.
        if not bool(torch.isfinite(ledger.total.detach())):
            invalid_groups = [
                name for name, value in ledger.groups.items()
                if not bool(torch.isfinite(value.detach()))
            ]
            raise FloatingPointError(
                "non-finite loss before backward/optimizer step: "
                + (", ".join(invalid_groups) or "total")
            )
