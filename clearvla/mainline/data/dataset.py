...
class ObservedStateDatasetConfig:
    ...
    def validate(self) -> None:
        ...
        if self.causal_reset_padding and (
        ...
        if (
            ...
            and not self.causal_reset_padding
        ):
...
class ObservedStateWindowDataset(Dataset):
        ...
        *,
        ...
        if allowed_boundary_regions is not None:
            ...
            if (
                ...
                raise ValueError(
                    "allowed boundary regions must be non-empty and unique"
                )
            unknown_regions = sorted(set(selected_regions) - set(BOUNDARY_REGIONS))
            if unknown_regions:
                raise ValueError(
                    f"unknown allowed boundary regions: {unknown_regions}"
                )
    ...
    def boundary_regions(self) -> np.ndarray:
        return np.asarray(
            [ref.boundary_region for ref in self.refs], dtype=object
        )
    ...
    def boundary_summary(self) -> dict[str, object]:
        ...
        source_action_counts = {
            int(index): int(self.boundary_plans[int(index)].source_action_count)
        ...
        if self.gripper_transition_boundary == "current_action_state":
            ...
            if previous_index < 0:
                if not self.config.causal_reset_padding or action_start != 0:
                    raise IndexError(
                        "previous-command gripper boundary precedes the episode"
                    )
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        if cfg.causal_reset_padding:
            # Match deployment history: repeat reset observations but never
            # pretend action[0] was executed before the first decision.
        ...
        if cfg.causal_reset_padding:
            executed_action_raw = np.asarray(
                actions_raw[np.maximum(executed_indices, 0)], dtype=np.float32
            ).copy()
            executed_action_raw[executed_indices < 0] = action_states_raw[0]
        else:
            executed_action_raw = np.asarray(
                actions_raw[executed_indices], dtype=np.float32
            )
