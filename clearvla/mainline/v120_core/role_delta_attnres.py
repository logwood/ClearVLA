"""Typed delta routing at the boundaries of the 3-3-2 role hierarchy.
...
class AffineVarianceFlooredCenteredNorm(nn.Module):
    ...
    def forward_with_denominator(
        ...
        bounded_weight = smooth_absolute_contract(
            self.weight, self.affine_maximum
        )
        output = (
            normalized.float()
            * bounded_weight.to(device=value.device, dtype=torch.float32)
            + self.bias.to(device=value.device, dtype=torch.float32)
        )
        gain = (
            bounded_weight.detach().float().abs().amax() / self.floor
        )
...
class PolicyRoleDeltaBank:
    ...
    def validate(self, *, hidden_size: int, horizon: int) -> None:
        if self.values.ndim != 5:
            raise ValueError(
                "policy role-delta values must be [B,source,horizon,basis,H]"
            )
    ...
    def forward(
        ...
        *,
        ...
        if source_count <= 0 or source_count > self.max_sources:
            raise ValueError(
                f"role-delta source count must be in [1,{self.max_sources}]"
            )
        ...
        if self.normalization_floor is None:
            query_unit = F.layer_norm(
                query.float(), (self.hidden_size,)
            ).to(dtype=query.dtype)
            value_unit = F.layer_norm(
                routed_values.float(), (self.hidden_size,)
            ).to(dtype=routed_values.dtype)
            query_denominator = query.new_ones(
                (*query.shape[:-1], 1), dtype=torch.float32
            )
        ...
        if self.include_null:
            ...
            source_probability = probabilities[..., :source_count].to(
        ...
        entropy_raw = -(
            probability_f * probability_f.clamp_min(1e-8).log()
        ).sum(dim=-1)
        ...
        source_entropy = -(
            conditional_source_probability
            * conditional_source_probability.clamp_min(1e-8).log()
        ).sum(dim=-1)
        ...
        metrics = {
            ...
            "null_mass": (
                probability_f[..., -1].mean()
                if self.include_null
                else probability_f.new_zeros(())
            ),
            ...
            "sample_route_std": probability_f.std(
