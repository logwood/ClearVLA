...
class _UtilityPrecisionLocalRefiner(_FunctionalOwnershipLocalRefiner):
    """Protected factual read with independent typed precision owners.
    ...
    gives an exact-zero policy update. Each optional value keeps its own typed
    read and output projection. The existing bounded router decides how to mix
    these values; they are not subtracted from the policy read beforehand.
    """

    refinement_contract = "independent_typed_precision_v1"
    ...
    def forward(
        ...
        *,
        ...
        typed_reads = {
            ...
            / math.sqrt(2.0),
            "semantic": typed_lane_reads[:, :, 1, owner_index["semantic"]],
            "appearance": typed_lane_reads[:, :, 0, owner_index["appearance"]],
            "geometry": (
                typed_lane_reads[:, :, 0, owner_index["geometry"]]
                + typed_lane_reads[:, :, 1, owner_index["geometry"]]
            )
