    ...
    def spatial_select(
        ...
        *,
        ...
        if self.spatial_intent_mode == "shared_target_prior_v1":
            ...
            target_address_supported = torch.where(
                semantic_interval_support,
                target_address_primary,
                torch.zeros_like(target_address_primary),
            )
            target_address_condition = target_address_supported
        else:
            target_address_supported = torch.zeros_like(target_address_primary)
        ...
        return selected, {
            **gradient_metrics,
            ...
            "object_p2_target_address_logit_rms": target_address_supported.detach()
            .square()
            .mean()
            .sqrt(),
            ...
            .mean()
            .sqrt(),
            "object_p2_target_address_k_center_error": (
                target_address_supported.detach().sum(dim=-1)
                / semantic_interval_support.detach().float().sum(dim=-1).clamp_min(1.0)
            )
            .abs()
            .amax(),
