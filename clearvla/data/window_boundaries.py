...
def resolve_window_boundary_plan(
    ...
    *,
    ...
    if selected == CAUSAL_PREFIX_V1:
        ...
        if (
            metadata.valid_center_start != 0
            or metadata.valid_center_end != int(complete_future_end)
        ):
            raise ValueError(
                "causal_prefix_v1 requires converter-declared valid bounds from "
                "reset through the final complete-future center"
            )
        if (
            metadata.strict_valid_center_start != int(strict_computed_start)
            or metadata.strict_valid_center_end != int(complete_future_end)
        ):
            raise ValueError(
                "causal_prefix_v1 requires independent converter-declared strict bounds"
            )
        if strict_start <= 0:
