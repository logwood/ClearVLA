...
class ClearVLACheckpointPolicy:
    ...
    def act_with_input(
        ...
        if output_mode == "calvin_binary_command":
            ...
            if command is None:
                raise RuntimeError(
                    "CALVIN binary deployment returned no command-state sample"
                )
            command_np = command[0].detach().float().cpu().numpy()
            if command_np.shape != (config.dimensions.action_horizon,):
                raise ValueError("CALVIN command-state sample has the wrong horizon")
            if not np.isfinite(command_np).all() or not np.isin(
                command_np, (-1.0, 1.0)
            ).all():
                raise ValueError("CALVIN command-state sample must contain only {-1,+1}")
